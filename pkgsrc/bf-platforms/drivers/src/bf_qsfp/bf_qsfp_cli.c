...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_get_pres(ucli_context_t *uc) {
  uint32_t lower_ports, upper_ports, cpu_ports;

  UCLI_COMMAND_INFO(uc, "get-pres", 0, "get-pres");
  if (bf_qsfp_get_transceiver_pres(&lower_ports, &upper_ports, &cpu_ports)) {
    aim_printf(&uc->pvs, "error getting qsfp presence\n");
    return 0;
  }
  aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_get_int(ucli_context_t *uc) {
  uint32_t lower_ports, upper_ports, cpu_ports;

  UCLI_COMMAND_INFO(uc, "get-int", 0, "get-int");
  bf_qsfp_get_transceiver_int(&lower_ports, &upper_ports, &cpu_ports);
  aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_get_lpmode(ucli_context_t *uc) {
  uint32_t lower_ports, upper_ports, cpu_ports;

  UCLI_COMMAND_INFO(uc, "get-lpmode", 0, "get-lpmode");
  if (bf_qsfp_get_transceiver_lpmode(&lower_ports, &upper_ports, &cpu_ports)) {
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_set_lpmode(ucli_context_t *uc) {
  int port, lpmode;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(
      uc, "set-lpmode", 2, "set-lpmode <port> <1: lpmode 0 : no lpmode>");
  ...
  if (bf_qsfp_set_transceiver_lpmode(port, (lpmode ? true : false))) {
    aim_printf(&uc->pvs, "error setting the lpmode\n");
  } else {
    aim_printf(&uc->pvs, "qsfp port %d lpmode set to %d\n", port, lpmode);
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_init(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-init", 0, "qsfp-init");
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_get_xver_info(
    ucli_context_t *uc) {
  int port;
  int max_port = bf_qsfp_get_max_qsfp_ports();
  qsfp_transciever_info_t info;

  UCLI_COMMAND_INFO(uc, "get-xver-info", 1, "get-xver-info <port>");
  port = atoi(uc->pargs->args[0]);
  if (port < 1 || port > max_port) {
    aim_printf(&uc->pvs, "port must be 1-%d\n", max_port);
    return 0;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_tx_disable_set(
    ucli_context_t *uc) {
  int port, ch, val, max_media_ch;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(
      uc, "tx-disable-set", 3, "tx-disable-set <port> <ch> <0:off 1:on>");
  port = atoi(uc->pargs->args[0]);
  if (port < 1 || port > max_port) {
    aim_printf(&uc->pvs, "port must be 1-%d\n", max_port);
    return 0;
  ...
  if ((val < 0) || (val > 1)) {
    aim_printf(&uc->pvs, "val must be 0-1 for port %d\n", port);
    return 0;
  }
...
static ucli_status_t qsfp_dump_pg0_info(ucli_context_t *uc, bool detail) {
  ...
  for (port = first_port; port <= last_port; port++) {
    ...
    qsfp_oper_info_get(port, &present[port], pg0_lower, pg0_upper);

    if (!present[port]) continue;
  ...
  aim_printf(&uc->pvs,
             ...
             "+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+-"
             "-+\n");
  aim_printf(&uc->pvs, "  2: [1:1]: IntL                  : ");
  dump_qsfp_oper_array(intl);
  aim_printf(&uc->pvs, "  2: [0:0]: Data_Not_Ready        : ");
  dump_qsfp_oper_array(dnr);
  aim_printf(&uc->pvs, "  3: [7:4]: L-Tx4-1 LOS           : ");
  dump_qsfp_oper_array(tx_los);
  aim_printf(&uc->pvs, "  3: [3:0]: L-Rx4-1 LOS           : ");
  dump_qsfp_oper_array(rx_los);
  aim_printf(&uc->pvs, "  4: [7:4]: L-Tx4-1 Adapt EQ Fault: ");
  dump_qsfp_oper_array(tx_eq_flt);
  aim_printf(&uc->pvs, "  4: [3:0]: L-Rx4-1 Adapt EQ Fault: ");
  dump_qsfp_oper_array(rx_eq_flt);
  aim_printf(&uc->pvs, "  5: [7:4]: L-Tx4-1 LOL           : ");
  dump_qsfp_oper_array(tx_lol);
  aim_printf(&uc->pvs, "  5: [3:0]: L-Rx4-1 LOL           : ");
  dump_qsfp_oper_array(rx_lol);
  aim_printf(&uc->pvs, " 86: [3:0]: Tx4-1 Disable         : ");
  dump_qsfp_oper_array(tx_dis);
  aim_printf(&uc->pvs, " 93: [1:1]: Power                 : ");
  dump_qsfp_oper_array(pwr);
  aim_printf(&uc->pvs, " 98: [7:4]: Tx4-1_CDR_control     : ");
  dump_qsfp_oper_array(tx_cdr_en);
  aim_printf(&uc->pvs, " 98: [3:0]: Rx4-1_CDR_control     : ");
  dump_qsfp_oper_array(rx_cdr_en);
  aim_printf(&uc->pvs, "234: [7:0]: Input Eq    (dB)      : ");
  dump_qsfp_oper_array(in_eq_lo);
  aim_printf(&uc->pvs, "235: [7:0]: Input Eq    (dB)      : ");
  dump_qsfp_oper_array(in_eq_hi);
  aim_printf(&uc->pvs, "236: [7:0]: Output Emph (dB)      : ");
  dump_qsfp_oper_array(out_emph_lo);
  aim_printf(&uc->pvs, "237: [7:0]: Output Emph (dB)      : ");
  dump_qsfp_oper_array(out_emph_hi);
  aim_printf(&uc->pvs, "238: [7:0]: Output Amp  (dB)      : ");
  dump_qsfp_oper_array(out_amp_lo);
  aim_printf(&uc->pvs, "239: [7:0]: Output Amp  (dB)      : ");
  dump_qsfp_oper_array(out_amp_hi);
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_pg0(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "pg0", -1, "pg0 [port]");
  qsfp_dump_pg0_info(uc, true /*detail*/);
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_oper(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "oper", -1, "oper [port]");
  qsfp_dump_pg0_info(uc, false /*summary*/);
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_pg3(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "pg3", -1, "pg3 [port]");

  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_dump_idprom_cache(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "dump-idprom-cache", 2, "<port> <page>");
  ...
  if (rc < 0) {
    aim_printf(&uc->pvs, "bf_qsfp_get_cached_info returned error\n");
    return 0;
  }
...
uint8_t *sfp_string(uint8_t *buf, size_t offset, size_t len) {
...
void printChannelMonitor(ucli_context_t *uc,
  ...
  uint16_t txpwrValue = (buf[txpwrMSB] << 8) | buf[txpwrLSB];
...
void printDdm_for_cmis(ucli_context_t *uc, unsigned int port) {
  double temp;
  double volt;
  double txBias;
  double txPower;
  ...
  bf_qsfp_field_read_onebank(port, TEMPERATURE, 0, 0, 2, &tem[0]);
  bf_qsfp_field_read_onebank(port, VCC, 0, 0, 2, &vcc[0]);

  // TBD - use onebank to get flags
  bf_qsfp_module_read(
      port, QSFP_BANK0, QSFP_PAGE17, 128, 128, buf + MAX_QSFP_PAGE_SIZE);

  temp = (tem[0]) + (tem[1] / 256.0);
  volt = ((vcc[0] << 8) | vcc[1]) / 10000.0;

  uint8_t max_media_ch = bf_qsfp_get_media_ch_cnt(port);

  for (uint8_t i = 0; i < max_media_ch; i++) {
    txBiasValue = (buf[170 + i * 2] << 8) | buf[171 + i * 2];
    txValue = (buf[154 + i * 2] << 8) | buf[155 + i * 2];
    rxValue = (buf[186 + i * 2] << 8) | buf[187 + i * 2];
...
void printThreshold(ucli_context_t *uc, unsigned int port) {
  ...
  if (bf_qsfp_get_threshold_info(port, &thresh)) {
    ...
    aim_printf(&uc->pvs, "\n");
    aim_printf(
        &uc->pvs, "%-20s = %fC\n", "TempHighAlarm", thresh.temp.alarm.high);
    aim_printf(
        &uc->pvs, "%-20s = %fC\n", "TempLowAlarm", thresh.temp.alarm.low);
    aim_printf(
        &uc->pvs, "%-20s = %fC\n", "TempHighWarning", thresh.temp.warn.high);
    aim_printf(
        &uc->pvs, "%-20s = %fC\n", "TempLowWarning", thresh.temp.warn.low);
    ...
    aim_printf(
        &uc->pvs, "%-20s = %0.2fVolts\n", "VccLowAlarm", thresh.vcc.alarm.low);
    aim_printf(&uc->pvs,
               "%-20s = %0.2fVolts\n",
               "VccHighWarning",
               thresh.vcc.warn.high);
    aim_printf(
        &uc->pvs, "%-20s = %0.2fVolts\n", "VccLowWarning", thresh.vcc.warn.low);
...
void printDdm(ucli_context_t *uc, unsigned int port, const uint8_t *buf) {
  double temp;
  double volt;
  double txBias;
  double txPower;
  ...
  for (int i = 0; i < 4; i++) {
    txBiasValue = (buf[42 + i * 2] << 8) | buf[43 + i * 2];
    txValue = (buf[50 + i * 2] << 8) | buf[51 + i * 2];
    rxValue = (buf[34 + i * 2] << 8) | buf[35 + i * 2];
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_get_ddm(ucli_context_t *uc) {
  uint8_t buf[MAX_QSFP_PAGE_SIZE * 2];
  int port_start, port_end;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(uc, "get-ddm", 2, "get-ddm <port_start> <max_port>");
  ...
  for (int i = port_start; i <= port_end; i++) {
    if (!bf_qsfp_is_present(i) && (!bf_qsfp_is_optical(i))) continue;
    ...
    if (bf_qsfp_is_cmis(i)) {
      ...
      // TBD - use onebank to get flags
      bf_qsfp_module_read(
          i, QSFP_BANK0, QSFP_PAGE0_LOWER, 0, MAX_QSFP_PAGE_SIZE_255, buf);
      printDdm(uc, i, buf);
    }
...
static void qsfp_type_to_display_get(bf_pltfm_qsfp_type_t qsfp_type,
                                     char *str,
                                     int len) {
  switch (qsfp_type) {
...
char *qsfp_power_class(uint8_t B129) {
  if ((B129 & 0x3) == 3) return " 7 (5.0 W max.)";
  if ((B129 & 0x3) == 2) return " 6 (4.5 W max.)";
  if ((B129 & 0x3) == 1) return " 5 (4.0 W max.)";
  if ((B129 & 0xC0) == 0xC0) return " 4 (3.5 W max.)";
  if ((B129 & 0xC0) == 0x80) return " 3 (2.5 W max.)";
  if ((B129 & 0xC0) == 0x40) return " 2 (2.0 W max.)";
  if ((B129 & 0xC0) == 0x00) return " 1 (1.5 W max.)";
  return "...           ";
}
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_summary(ucli_context_t *uc) {
  uint8_t buf[MAX_QSFP_PAGE_SIZE * 2];
  int port;
  ...
  memset((char *)conn_serial_nbr, 0, sizeof(conn_serial_nbr));

  for (port = 1; port <= max_port; port++) {
    if (!bf_qsfp_is_present(port)) continue;

    if (bf_qsfp_get_cached_info(port, QSFP_PAGE0_LOWER, buf)) {
      continue;
    }
    if (bf_qsfp_get_cached_info(
            port, QSFP_PAGE0_UPPER, buf + MAX_QSFP_PAGE_SIZE)) {
      continue;
    }
    ...
    bf_qsfp_get_module_type_str(port, module_type_str, false);
    ...
    aim_printf(&uc->pvs, "  %02x:%02x:%02x", buf[145], buf[146], buf[147]);
    aim_printf(&uc->pvs, "  %s ", qsfpdd_power_class(buf[200]));

    aim_printf(&uc->pvs, "   %s\t", bf_cmis_get_media_type_string(port));
  ...
  for (port = 1; port <= max_port; port++) {
    int other_port;
    for (other_port = port + 1; other_port < max_port; other_port++) {
      // skip missing qsfps
      if (conn_serial_nbr[port][0] == 0) continue;
      if (conn_serial_nbr[other_port][0] == 0) continue;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_summary(ucli_context_t *uc) {
  uint8_t buf[MAX_QSFP_PAGE_SIZE * 2];
  int port;
  int max_port = bf_qsfp_get_max_qsfp_ports();
  ...
  for (port = 1; port <= max_port; port++) {
    ...
    aim_printf(&uc->pvs, "%s ", vendor_sn);
    aim_printf(&uc->pvs, "%s ", vendor_date);
    aim_printf(&uc->pvs, "%5d MBps ", buf[140] * 100);
    aim_printf(&uc->pvs,
               "  %02x:%02x:%02x",
               buf[165 - 128],
               buf[166 - 128],
               buf[167 - 128]);
    aim_printf(&uc->pvs, "  %s ", qsfp_power_class(buf[129]));
    if (bf_qsfp_is_optical(port)) {
      if (buf[146]) aim_printf(&uc->pvs, "  (Optical): %d m ", buf[146]);
    } else {
      if (buf[146]) aim_printf(&uc->pvs, "  (Copper): %d m ", buf[146]);
    }
    if (buf[142]) aim_printf(&uc->pvs, "  (SMF): %d km ", buf[142]);
    if (buf[143]) aim_printf(&uc->pvs, "  (OM3): %d m ", buf[143] * 2);
    if (buf[144]) aim_printf(&uc->pvs, "  (OM2): %d m ", buf[144]);
    if (buf[145]) aim_printf(&uc->pvs, "  (OM1): %d m ", buf[145]);
    aim_printf(&uc->pvs, "\n");
  }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_show_for_copper(
    ucli_context_t *uc) {
  ...
  for (port = 1; port <= max_port; port++) {
    if (!bf_qsfp_is_present(port)) continue;
    if (!bf_qsfp_is_cmis(port)) continue;
    if (bf_qsfp_is_optical(port)) continue;

    // grab the cached vendor info
    if (!bf_qsfp_get_vendor_info(port, &vendor)) {
      continue;
    }
    if (!bf_qsfp_get_module_type_str(port, module_type_str, true)) {
      continue;
    }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_show(ucli_context_t *uc) {
  qsfp_vendor_info_t vendor;
  char module_type_str[40];
  int port;
  int max_port = bf_qsfp_get_max_qsfp_ports();
  bf_pltfm_qsfpdd_type_t qsfpdd_type = BF_PLTFM_QSFPDD_UNKNOWN;
...
void printChannelMonitor_for_dd(ucli_context_t *uc,
                                unsigned int index,
                                const uint8_t *buf,
                                unsigned int rxMSB,
                                unsigned int rxLSB,
  ...
  /* TX bias ranges from 0mA to 131mA */
  double txBias = ((double)(txbias_mul * txValue)) / 65535;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_show(ucli_context_t *uc) {
  uint8_t buf[MAX_QSFP_PAGE_SIZE * 2];
  int port;
  int max_port = bf_qsfp_get_max_qsfp_ports();
  uint8_t vendor[17];
  ...
  UCLI_COMMAND_INFO(uc, "show", 0, "show qsfp or qsfp-dd summary information");

  aim_printf(&uc->pvs, "Max ports supported ==> %2d", max_port);
  aim_printf(&uc->pvs, "\n================\n");
  ...
  for (port = 1; port <= max_port; port++) {
    ...
    if (buf[142]) {
      snprintf(optical_lstr, sizeof(optical_lstr), "SMF %d km", buf[142]);
    } else if (buf[143]) {
      snprintf(optical_lstr, sizeof(optical_lstr), "OM3 %d m", buf[143] * 2);
    } else if (buf[144]) {
      snprintf(optical_lstr, sizeof(optical_lstr), "OM2 %d m", buf[144]);
    } else if (buf[145]) {
      snprintf(optical_lstr, sizeof(optical_lstr), "OM1 %d m", buf[145]);
    } else {
      snprintf(optical_lstr, sizeof(optical_lstr), "--0--");
    }
    /* get qsfp_type */
    ...
    qsfp_type_to_display_get(
        qsfp_type, display_qsfp_type, sizeof(display_qsfp_type));
    /* now display the stuff */
    aim_printf(
        &uc->pvs, " %2d  0x%02x %16s %16s", port, buf[0], vendor, vendor_pn);
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfpdd_dump_info(ucli_context_t *uc) {
  ...
  UCLI_COMMAND_INFO(uc, "dump-info", 1, "dump-info <port>");
  port = atoi(uc->pargs->args[0]);
  if (port < 1 || port > max_port) {
    aim_printf(&uc->pvs, "port must be 1-%d\n", max_port);
    return 0;
  ...
  aim_printf(&uc->pvs, "## MODULE STATE MACHINE\n");
  bf_qsfp_get_module_state_info(port, &module_state_info);
  aim_printf(&uc->pvs,
             "%-30s = %s\n",
             "Module state",
  ...
  bf_cmis_spec_rev_get(port, &spec_major_rev, NULL);
  if (spec_major_rev >= 4) {
    aim_printf(
        &uc->pvs, "%-30s = %d\n", "LowPwr bit", module_state_info.lowPwr);
  }
  aim_printf(&uc->pvs, "\n");

  aim_printf(&uc->pvs, "## DATA PATH STATE MACHINE\n");
  int cur_ch, num_ch = bf_qsfp_get_ch_cnt(port);
  qsfp_datapath_state_info_t dp_st_info[num_ch];

  bf_qsfp_get_dp_state_info(port, num_ch, dp_st_info);
  ...
  for (cur_ch = 0; cur_ch < num_ch; cur_ch++) {
    aim_printf(&uc->pvs, "%-5s", dp_st_info[cur_ch].datapath_state_str_short);
  }
  aim_printf(&uc->pvs, "\n");
  if (spec_major_rev >= 4) {
    aim_printf(&uc->pvs, "%-25s", "DataPathDeinit bits");
  } else {
    aim_printf(&uc->pvs, "%-25s", "DataPathPwrUp bits");
  }
  for (cur_ch = 0; cur_ch < num_ch; cur_ch++) {
    aim_printf(&uc->pvs, "%-5d", dp_st_info[cur_ch].data_path_deinit_bit);
  }
  aim_printf(&uc->pvs, "\n\n");

  // TBD - use onebank to get flags
  bf_qsfp_module_read(
      port, QSFP_BANK0, QSFP_PAGE17, 128, 128, buf17 + MAX_QSFP_PAGE_SIZE);

  aim_printf(&uc->pvs, "## LATCHED FLAGS\n");
  aim_printf(&uc->pvs, "TX Latched Flags (Stop periodics and read twice):\n");
  aim_printf(&uc->pvs, "  TX-Fault: 0x%0x \n", buf17[135]);
  aim_printf(&uc->pvs, "  TX-LOS: 0x%0x \n", buf17[136]);
  aim_printf(&uc->pvs, "  TX-CDR-LOL: 0x%0x \n", buf17[137]);
  aim_printf(&uc->pvs, "  TX-Adaptive Input Eq Fault: 0x%0x \n", buf17[138]);

  aim_printf(&uc->pvs, "  TX-output power High Alarm: 0x%0x \n", buf17[139]);
  aim_printf(&uc->pvs, "  TX-output power Low Alarm: 0x%0x \n", buf17[140]);
  aim_printf(&uc->pvs, "  TX-output power High warning: 0x%0x \n", buf17[141]);
  aim_printf(&uc->pvs, "  TX-output power Low warning: 0x%0x \n", buf17[142]);

  // if (!bf_qsfp_field_read_onebank(port, CHANNEL_TX_BIAS_ALARMS, 0, 0, 4,
  // &buf4[0])) {
  //}
  aim_printf(&uc->pvs, "  TX-Bias High Alarm: 0x%0x \n", buf17[143]);
  aim_printf(&uc->pvs, "  TX-Bias Low Alarm: 0x%0x \n", buf17[144]);
  aim_printf(&uc->pvs, "  TX-Bias High warning: 0x%0x \n", buf17[145]);
  aim_printf(&uc->pvs, "  TX-Bias Low warning: 0x%0x \n", buf17[146]);

  aim_printf(&uc->pvs, "RX Flags:\n");
  aim_printf(&uc->pvs, "  RX-LOS: 0x%0x \n", buf17[147]);
  aim_printf(&uc->pvs, "  RX-CDR-LOL: 0x%0x \n", buf17[148]);

  aim_printf(&uc->pvs, "  RX-input power High Alarm: 0x%0x \n", buf17[149]);
  aim_printf(&uc->pvs, "  RX-input power Low Alarm: 0x%0x \n", buf17[150]);
  aim_printf(&uc->pvs, "  RX-input power High warning: 0x%0x \n", buf17[151]);
  aim_printf(&uc->pvs, "  RX-input power Low warning: 0x%0x \n", buf17[152]);
  ...
  if (!bf_qsfp_field_read_onebank(port, PWR_CLASS, 0, 0, 1, &data)) {
    aim_printf(&uc->pvs, "  Power Class: %s \n", qsfpdd_power_class(data));
  }
  if (!bf_qsfp_field_read_onebank(port, TEMPERATURE, 0, 0, 2, &buf2[0])) {
    double temp = (buf2[0]) + (buf2[1] / 256.0);
    aim_printf(&uc->pvs, "  Temperature: %f C\n", temp);
  }
  if (!bf_qsfp_field_read_onebank(port, VCC, 0, 0, 2, &buf2[0])) {
    double voltage = ((buf2[0] << 8) | buf2[1]) / 10000.0;
  ...
  uint8_t rxPwrByte = 186, txPwrByte = 154, txBiasByte = 170;
  uint8_t i = 0;
  uint8_t max_media_ch = bf_qsfp_get_media_ch_cnt(port);
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_dump_info(ucli_context_t *uc) {
  uint8_t buf[MAX_QSFP_PAGE_SIZE * 2];
  int port;
  qsfp_vendor_info_t vendor;
  ...
  bf_qsfp_get_module_type_str(port, module_type_str, true);
  aim_printf(
  ...
  if (bf_qsfp_is_cmis(port)) {
    ...
    bf_qsfp_get_sff_eth_extended_code_description(port, ext_code_desc);
    aim_printf(&uc->pvs, "%-30s = %s\n", "Module", ext_code_desc);
  }
  aim_printf(&uc->pvs, "\n");
  ...
  // TBD - use onebank to get flags
  bf_qsfp_module_read(port, QSFP_BANK0, QSFP_PAGE0_LOWER, 0, 128, buf);
  bf_qsfp_module_read(
      port, QSFP_BANK0, QSFP_PAGE0_UPPER, 128, 128, buf + MAX_QSFP_PAGE_SIZE);
  ...
  aim_printf(&uc->pvs, "  Nominal Bit Rate: %d MBps\n", buf[140] * 100);
  aim_printf(&uc->pvs, "  Ext rate select compliance: 0x%#02x\n", buf[141]);
  ...
  aim_printf(&uc->pvs, "  Enhanced Options: 0x%02x\n", buf[221]);
  ...
  aim_printf(&uc->pvs, "    Rx Power: 0x%02x 0x%02x\n", buf[9], buf[10]);
  aim_printf(&uc->pvs, "    Tx Bias: 0x%02x 0x%02x\n", buf[11], buf[12]);
  aim_printf(&uc->pvs, "    Reserved Set 3: 0x%02x 0x%02x\n", buf[13], buf[14]);
  aim_printf(&uc->pvs, "    Reserved Set 4: 0x%02x 0x%02x\n", buf[15], buf[16]);
  aim_printf(&uc->pvs, "    Reserved Set 5: 0x%02x 0x%02x\n", buf[17], buf[18]);
  aim_printf(&uc->pvs,
  ...
  aim_printf(&uc->pvs, "\n");
  aim_printf(&uc->pvs, "## POWER/TEMPERATURE\n");
  aim_printf(&uc->pvs, "  Power set/LPMODE:  0x%02x\n", buf[93]);
  aim_printf(&uc->pvs, "  TX disable bits: 0x%02x\n", buf[86]);
  aim_printf(&uc->pvs, "  TX rate select bits: 0x%02x\n", buf[88]);
  ...
  aim_printf(&uc->pvs,
             ...
             "TX Power",
             "TX Bias");
  printChannelMonitor(uc, 1, buf, 34, 35, 42, 43, 50, 51);
  printChannelMonitor(uc, 2, buf, 36, 37, 44, 45, 52, 53);
  printChannelMonitor(uc, 3, buf, 38, 39, 46, 47, 54, 55);
  printChannelMonitor(uc, 4, buf, 40, 41, 48, 49, 56, 57);
  aim_printf(&uc->pvs,
...
static int bf_pltfm_ucli_ucli__qsfp_mem_show_dp(
  ...
  aim_printf(&uc->pvs, "%-25s", "ApSel code selected");
  for (cur_ch = 0; cur_ch < num_ch; cur_ch++) {
    aim_printf(&uc->pvs, "%-5d", dp_ss_info[cur_ch].selected_ApSel_code);
  }
  aim_printf(&uc->pvs, "\n");

  aim_printf(&uc->pvs, "%-25s", "Data path ID (1stLn-1)");
  for (cur_ch = 0; cur_ch < num_ch; cur_ch++) {
    aim_printf(&uc->pvs, "%-5d", dp_ss_info[cur_ch].data_path_id);
  }
  aim_printf(&uc->pvs, "\n");
  return 0;
}

static int bf_pltfm_ucli_ucli__qsfp_module_show(ucli_context_t *uc, int port) {
  aim_printf(&uc->pvs, "Module memory dump, port %d\n", port);
  ...
  aim_printf(&uc->pvs, "%-30s = ", "Memory map format");
  if (is_cmis) {
  ...
  aim_printf(&uc->pvs, "%-30s = ", "Flat/paged memory");
  if (is_flat_mem) {
  ...
  if ((!is_passive_cu) && (bf_qsfp_get_firmware_ver(port,
                                                    &active_ver_major,
                                                    &active_ver_minor,
                                                    &inactive_ver_major,
                                                    &inactive_ver_minor))) {
    ...
    bf_qsfp_get_hardware_ver(port, &hw_ver_major, &hw_ver_minor);
    aim_printf(&uc->pvs,
               "%-30s = %d.%d\n",
               "Hardware version",
               hw_ver_major,
  ...
  aim_printf(&uc->pvs, "%-40s%-4s", "Standard supported", "Lns");
  if ((is_cmis) && (!is_passive_cu)) {
    aim_printf(&uc->pvs,
               "%-4s  %-40s%-4s%s",
               "Mask",
  ...
  int app_count = bf_qsfp_get_application_count(port);
  qsfp_application_info_t app_info[app_count];
  if (app_count == 0) {
    aim_printf(&uc->pvs, "NONE ADVERTISED\n");
  } else {
    for (int cur_app = 0; cur_app < app_count; cur_app++) {
      bf_qsfp_get_application(port, cur_app + 1, &app_info[cur_app]);
      ...
      if ((is_cmis) && (!is_passive_cu)) {
        aim_printf(&uc->pvs, "0x%02X", app_info[cur_app].host_lane_assign_mask);
        aim_printf(&uc->pvs,
                   "  %-40s%-4d",
                   app_info[cur_app].media_if_id_str,
                   app_info[cur_app].media_lane_cnt);
        aim_printf(
            &uc->pvs, "0x%02X", app_info[cur_app].media_lane_assign_mask);
      }
      aim_printf(&uc->pvs, "\n");
    }
    aim_printf(&uc->pvs, "\n");
  ...
  if ((is_cmis) && (!is_passive_cu)) {
    ...
    qsfp_datapath_state_info_t dp_st_info[num_ch];
    qsfp_datapath_staged_set_info_t cfg_active_set[num_ch];
    qsfp_datapath_staged_set_info_t cfg_staged_set0[num_ch];
    ...
    bf_qsfp_get_dp_state_config(port, num_ch, cfg_active_set, ACTIVE_SET);
    bf_pltfm_ucli_ucli__qsfp_mem_show_dp(
        uc, app_info, cfg_active_set, ACTIVE_SET, num_ch);
    aim_printf(&uc->pvs, "\n");

    // Staged set 0 Data Path info
    bf_qsfp_get_dp_state_config(port, num_ch, cfg_staged_set0, STAGED_SET0);
    bf_pltfm_ucli_ucli__qsfp_mem_show_dp(
        uc, app_info, cfg_staged_set0, STAGED_SET0, num_ch);
    aim_printf(&uc->pvs, "\n");
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_mem_show(ucli_context_t *uc) {
  int startport, curport;
  UCLI_COMMAND_INFO(uc, "module-show", 1, "module-show <port>");
  int max_port = bf_qsfp_get_max_qsfp_ports();

  curport = atoi(uc->pargs->args[0]);
  if (curport > max_port) {
    aim_printf(&uc->pvs, "port must be 1-%d or -1 for all\n", max_port);
    return -1;
  }
  ...
  for (curport = startport; curport <= max_port; curport++) {
    if (bf_qsfp_is_present(curport)) {
      bf_pltfm_ucli_ucli__qsfp_module_show(uc, curport);
      if (curport != max_port) {
        aim_printf(&uc->pvs, "\n");
      }
    }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_detect_xver(ucli_context_t *uc) {
  int port;
  bool is_present;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(uc, "detect-xver", 1, "detect-xver <port>");

  port = atoi(uc->pargs->args[0]);
  if (port > 0) {
    if (bf_qsfp_detect_transceiver(port, &is_present) == 0) {
      aim_printf(&uc->pvs,
                 "port %d %s\n",
                 port,
    ...
    int i, cnt = max_port;
    for (i = 0; i < cnt; i++) {
      if (bf_qsfp_detect_transceiver(i, &is_present) == 0) {
        aim_printf(
            &uc->pvs, "port %d %s\n", i, (is_present ? "detected" : "removed"));
      } else {
        aim_printf(&uc->pvs, "error detecting port %d\n", i);
      }
    }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_type_show(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-type", 1, "qsfp-type <port>");
  int port;
  bf_pltfm_qsfp_type_t qsfp_type = BF_PLTFM_QSFP_UNKNOWN;
  char display_qsfp_type[13];
  ...
  for (port = port_begin; port <= max_port; port++) {
    if (bf_qsfp_type_get(port, &qsfp_type) != 0) {
      qsfp_type = BF_PLTFM_QSFP_UNKNOWN;
    }
    qsfp_type_to_display_get(
        qsfp_type, display_qsfp_type, sizeof(display_qsfp_type));
    aim_printf(&uc->pvs, "port %d %s\n", port, display_qsfp_type);
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_read_reg(ucli_context_t *uc) {
  int port, page, offset;
  uint8_t val;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(uc, "read-reg", 3, "read-reg <port> <page> <offset>");
  port = atoi(uc->pargs->args[0]);
  ...
  // this routine currently assumes bank 0
  if (bf_qsfp_module_read(port, QSFP_BANK0, page, offset, 1, &val)) {
    aim_printf(&uc->pvs, "error reading register\n");
  } else {
    aim_printf(&uc->pvs, "page %d offset 0x%x read 0x%0x\n", page, offset, val);
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_reset(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc, "qsfp-reset", 2, "qsfp-reset <port> <1=reset, 0=unreset>");
  int port;
  int port_begin, max_port;
  int reset_val;
  bool reset;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_write_reg(ucli_context_t *uc) {
  int port, page, offset;
  uint8_t val;
  int max_port = bf_qsfp_get_max_qsfp_ports();

  UCLI_COMMAND_INFO(
      uc, "write-reg", 4, "write-reg <port> <page> <offset> <val>");
  port = atoi(uc->pargs->args[0]);
  page = atoi(uc->pargs->args[1]);
  offset = strtol(uc->pargs->args[2], NULL, 0);
  ...
  // this routine currently assumes bank 0
  if (bf_qsfp_module_write(port, QSFP_BANK0, page, offset, 1, &val)) {
    aim_printf(&uc->pvs, "error writing register\n");
  } else {
    aim_printf(
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_lpmode_hw(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc, "qsfp-lpmode-hw", 2, "qsfp-lpmode-hw <port> <1=reset, 0=unreset>");
  int port;
  int port_begin, max_port;
  int lpmode_val;
  bool lpmode;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_lpmode_sw(ucli_context_t *uc) {
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_lpbk(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-lpbk", 2, "qsfp-lpbk port [near/elec far/opt]");
  int port;
  bool lpbk_near = true;
  int max_port = bf_qsfp_get_max_qsfp_ports();
  ...
  if (port > max_port) return 0;
  qsfp_luxtera_lpbk(port, lpbk_near);
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_capture(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-capture", 1, "qsfp-capture port");
  int port, ofs;
  ...
  for (ofs = 0; ofs < 12288; ofs++) {
    if ((ofs % 16) == 0) {
      printable_strs[16] = 0;
      aim_printf(&uc->pvs, " %s\n%6d : ", printable_strs, ofs);
    }
    aim_printf(&uc->pvs, "%02x ", arr_0x3k[ofs]);
    printable_strs[(ofs % 16)] = isprint(arr_0x3k[ofs]) ? arr_0x3k[ofs] : '.';
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_tmr_stop(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-tmr-stop", 0, "qsfp-tmr-stop");
  bf_pltfm_pm_qsfp_scan_poll_stop();
  bf_sys_sleep(1); /* wait for on going qsfp scan to be over */
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_tmr_start(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-tmr-start", 0, "qsfp-tmr-start");
  bf_pltfm_pm_qsfp_scan_poll_start();
  return 0;
}

static int bf_qsfp_ucli_qsfp_type_get(const char *str1,
                                      bf_pltfm_qsfp_type_t *qsfp_type) {
  int i = 0;
  char str[25];

  if ((!qsfp_type) || (!str1)) return -1;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_special_case_set(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "qsfp-type-set",
                    2,
  ...
  ret = bf_qsfp_ucli_qsfp_type_get(uc->pargs->args[1], &qsfp_type);
  if (ret != 0) {
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_special_case_clear(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc, "qsfp-type-clear", 1, "qsfp-type-clear <port> ");
  int port;
  int port_begin, max_port;
  ...
  for (port = port_begin; port <= max_port; port++) {
    bf_qsfp_special_case_set(port, BF_PLTFM_QSFP_UNKNOWN, false);
  }
  return 0;
}

static ucli_status_t bf_pltfm_ucli_ucli__qsfp_soft_remove(ucli_context_t *uc) {
  ...
  if ((port_begin == 0) || (port_begin > max_ports || max_port > max_ports)) {
    aim_printf(&uc->pvs,
  ...
  if (strcmp(uc->pargs->args[1], "set") == 0) {
    remove_flag = true;
  } else if (strcmp(uc->pargs->args[1], "clear") == 0) {
    remove_flag = false;
  } else {
    aim_printf(&uc->pvs,
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_soft_remove_show(
    ucli_context_t *uc) {
  int port, port_begin, max_port;
  int max_ports = bf_qsfp_get_max_qsfp_ports();
  int conn_id = 0;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_write_mul(ucli_context_t *uc) {
  ...
  if (uc->pargs->count < 5) {
    aim_printf(&uc->pvs, "Minimum 4 args required to write the data \n");
    return 0;
  }
  ...
  if (!nBytes) {
    aim_printf(&uc->pvs, "number of bytes cannot be zero\n");
    arg_err = 1;
  }

  int n = uc->pargs->count - 4;
  if (n != nBytes) {
    aim_printf(&uc->pvs, "Looks like number of arguments are wrong\n");
    arg_err = 1;
  }

  if (arg_err) return 0;

  val = (uint8_t *)bf_sys_calloc(nBytes, sizeof(uint8_t));
  ...
  if (bf_qsfp_module_write(port, QSFP_BANK0, page, offset, nBytes, val)) {
    aim_printf(&uc->pvs, "error writing the module %d\n", port);
  } else {
    aim_printf(&uc->pvs,
...
q_free:
  if (val) bf_sys_free(val);
  return 0;
}

static ucli_status_t qsfp_read(ucli_context_t *uc, bool is_cached_read) {
  ...
  if (uc->pargs->count < 4) {
    aim_printf(&uc->pvs, "Minimum 4 args required to read the data\n");
    return 0;
  }
  ...
  if (is_cached_read) {
    sts =
        bf_qsfp_module_cached_read(port, QSFP_BANK0, page, offset, nBytes, val);
  } else {
    sts = bf_qsfp_module_read(port, QSFP_BANK0, page, offset, nBytes, val);
  }
  if (sts != BF_SUCCESS) {
    aim_printf(&uc->pvs, "error reading the module %d\n", port);
    goto q_free;
  }
  ...
  if ((offset + i) % 16 != 0) {
    for (int k = 0; k < (16 - ((offset + i) % 16)); k++) {
      aim_printf(&uc->pvs, "   ");
      if (k < 17) {
        as_chars[k] = '.';
      }
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_read_mul(ucli_context_t *uc) {
  UCLI_COMMAND_INFO(uc,
                    "qsfp-read",
                    -1,
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_combined_read(
    ucli_context_t *uc) {
  UCLI_COMMAND_INFO(
      uc,
      "qsfp-combined-read",
...
static ucli_status_t bf_pltfm_ucli_ucli__mark_qsfp_port_as_internal(
    ucli_context_t *uc) {
  int port, port_begin, max_port;
  int max_ports = bf_qsfp_get_max_qsfp_ports();
  bool set = 0;
...
static ucli_status_t bf_pltfm_ucli_ucli__qsfp_fsm(ucli_context_t *uc) {
  int port, first_port, last_port, max_port;
  int i;

  UCLI_COMMAND_INFO(uc, "qsfp-fsm", -1, "qsfp-fsm <port>");
  ...
  aim_printf(&uc->pvs, "%-29s | ", "CH7");
  aim_printf(&uc->pvs, "%-29s | \n", "CH8");

  for (port = first_port; port <= last_port; port++) {
    aim_printf(&uc->pvs, "%-4d | %s | ", port, qsfp_module_fsm_st_get(port));
    for (i = 0; i < MAX_CHAN_PER_CONNECTOR; i++) {
      if (i >= bf_qsfp_get_ch_cnt(port))
        aim_printf(&uc->pvs, "%-29s | ", "NA");
      else
        aim_printf(&uc->pvs, "%-29s | ", qsfp_channel_fsm_st_get(port, i));
    }
    aim_printf(&uc->pvs, "\n");
  }
  return 0;
...
static ucli_command_handler_f bf_pltfm_qsfp_ucli_ucli_handlers__[] = {
...
ucli_node_t *bf_qsfp_ucli_node_create(ucli_node_t *m) {
  ...
  n = ucli_node_create("qsfp", m, &bf_pltfm_qsfp_ucli_module__);
  ucli_node_subnode_add(n, ucli_module_log_node_create("qsfp"));
  return n;
}
