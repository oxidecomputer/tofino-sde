...
typedef double (*qsfp_conversion_fn)(uint16_t value);
...
static bf_qsfp_info_t bf_qsfp_info_arr[BF_PLAT_MAX_QSFP + 1];
...
static bf_qsfp_flag_format_t sff8636_flag_formats[] = {
...
static cmis_app_host_id_map_t cmis_app_host_if_speed_map[] = {
...
// the host_id's in this array are from SFF-8024, rev 4.6, section 4.7
static cmis_app_host_id_map_t innolight_cmis_app_host_if_speed_map[] = {
...
// the host_id's in this array are from SFF-8024, rev 4.6, section 4.7
static cmis_app_media_id_map_t cmis_app_media_if_speed_map[] = {
...
// the host_id's in this array are from SFF-8024, rev 4.6, section 4.7
static cmis_app_media_id_map_t innolight_cmis_app_media_if_speed_map[] = {
    {MEDIA_TYPE_SMF, 251, "200GBASE-DR2     2 X 53.125 Gbd PAM4"},
    {MEDIA_TYPE_SMF, 252, "800GBASE-DR8     8 X 53.125 Gbd PAM4"},
};

static cmis_app_host_id_map_t *bf_qsfp_get_custom_host_if_speed_map(
  ...
  if (!strncmp(vendor.name, "INNOLIGHT", strlen("INNOLIGHT"))) {
    map_ptr = innolight_cmis_app_host_if_speed_map;
    array_size = sizeof(innolight_cmis_app_host_if_speed_map) /
...
static bf_port_speed_t bf_qsfp_decode_host_speed(int port, uint8_t host_id) {
  ...
  int array_ind = 0, array_size = sizeof(cmis_app_host_if_speed_map) /
                                  sizeof(cmis_app_host_id_map_t);
  bool matchfound = false;
  cmis_app_host_id_map_t *host_id_map;

  if (qsfp_needs_hi_pwr_init(port)) {
    array_size = 1;  // hardware init uses only first app
  }

  while ((array_ind < array_size) && (!matchfound)) {
    if (cmis_app_host_if_speed_map[array_ind].host_id == host_id) {
      matchfound = true;
      matchspeed = cmis_app_host_if_speed_map[array_ind].speed;
    } else {
      array_ind++;
    }
  }
  ...
  if (!matchfound) {
    ...
    host_id_map = bf_qsfp_get_custom_host_if_speed_map(port, host_id);
    if (host_id_map) {
...
static bool bf_qsfp_decode_host_id(int port,
  ...
  while ((array_ind < array_size) && (!matchfound)) {
    if (cmis_app_host_if_speed_map[array_ind].host_id == host_id) {
      matchfound = true;
      strcpy(host_id_desc, cmis_app_host_if_speed_map[array_ind].host_id_desc);
    } else {
      array_ind++;
    }
  }
...
static cmis_app_media_id_map_t *bf_qsfp_get_custom_media_if_speed_map(
  ...
  if (!strncmp(vendor.name, "INNOLIGHT", strlen("INNOLIGHT"))) {
    ...
    array_size = sizeof(innolight_cmis_app_media_if_speed_map) /
...
static bool bf_qsfp_decode_media_id(int port,
  ...
  // For 8636 supported compliance codes, the equivalent CMIS host id
  // is entered in the map
  int array_ind = 0, array_size = sizeof(cmis_app_media_if_speed_map) /
                                  sizeof(cmis_app_media_id_map_t);
  bool matchfound = false;
  ...
  while ((array_ind < array_size) && (!matchfound)) {
    if ((cmis_app_media_if_speed_map[array_ind].media_type == media_type) &&
        (cmis_app_media_if_speed_map[array_ind].media_id == media_id)) {
      matchfound = true;
  ...
  if (!matchfound) {
    // Check for custom host ID
    media_id_map =
        bf_qsfp_get_custom_media_if_speed_map(port, media_type, media_id);
    if (media_id_map) {
      matchfound = true;
      strcpy(media_id_desc, media_id_map->media_id_desc);
    }
  }
  if (!matchfound) {
    sprintf(media_id_desc, "Unknown (0x%X)", media_id);
  }
  return matchfound;
}
...
static int bf_qsfp_id_matching_app(int port,
  ...
  if (!qsfp_info) return -1;

  int cur_app = 0;
  bool matchfound = false;
  while ((cur_app < qsfp_info->num_apps) && (!matchfound)) {
    if (intf_lanes == qsfp_info->app_list[cur_app].host_lane_cnt) {
      if ((intf_speed != 0) &&
...
int bf_cmis_find_matching_Application(int port,
  ...
  rc = bf_qsfp_id_matching_app(port, speed, nlanes, &bf_qsfp_info_arr[port]);
...
int bf_cmis_get_Application_media_info(int port,
                                       ...
                                       int *media_firstLane,
                                       int *media_nLanes) {
  if ((!media_lane || !media_firstLane || !media_nLanes) ||
      (port > bf_plt_max_qsfp)) {
    return -1;
  }

  // range check application
  if ((curr_app < 1) || (curr_app > bf_qsfp_info_arr[port].num_apps)) {
    LOG_ERROR("QSFP    %2d : invalid ApSel (%d)", port, curr_app);
    return -1;
  }

  int app = curr_app - 1;  // Since ApSel is 1-based but app_list is 0 indexed

  if ((bf_qsfp_info_arr[port].app_list[app].media_lane_cnt) == 0) {
  ...
  if ((bf_qsfp_info_arr[port].app_list[app].host_lane_cnt) == 0) {
  ...
  if ((bf_qsfp_info_arr[port].app_list[app].host_lane_cnt %
       bf_qsfp_info_arr[port].app_list[app].media_lane_cnt) != 0) {
  ...
  *media_nLanes = bf_qsfp_info_arr[port].app_list[app].media_lane_cnt;

  // figure out which host lane group we're using
  int host_ln_group = 0, host_firstLane = 0, curlane;
  for (curlane = 0; curlane <= host_lane; curlane++) {
  ...
  curlane = 0;
  while ((media_ln_group < host_ln_group) && (curlane < 8)) {
  ...
  *media_lane = *media_firstLane +
                (int)((host_lane - host_firstLane) / host_media_lane_ratio);
  return 0;
}
...
int bf_qsfp_get_media_type_str(int port, char *media_type_str) {
  if (bf_qsfp_is_cmis(port)) {
    int array_ind = 0,
        array_size = sizeof(cmis_media_type_desc) / sizeof(cmis_media_type_t);
    bool matchfound = false;
    ...
    while ((array_ind < array_size) && (!matchfound)) {
      if (cmis_media_type_desc[array_ind].media_type ==
        ...
        matchfound = true;
        strcpy(media_type_str, cmis_media_type_desc[array_ind].media_type_str);
      } else {
        array_ind++;
      }
    }
...
int bf_qsfp_get_application(int port,
  ...
  bf_pm_qsfp_apps_t *cur_app = &bf_qsfp_info_arr[port].app_list[ApSel - 1];
  app_info->host_if_id = cur_app->host_if_id;
  app_info->media_if_id = cur_app->media_if_id;
  bf_qsfp_decode_host_id(port, cur_app->host_if_id, app_info->host_if_id_str);
...
int bf_qsfp_rxlos_debounce_set(int port, int count) {
  if (port > bf_plt_max_qsfp) {
    return -1;
  }
...
static bf_qsfp_flag_format_t *find_sff8636_flag_format(Sff_field fieldName) {
  int i, cnt = sizeof(sff8636_flag_formats) / sizeof(bf_qsfp_flag_format_t);

  for (i = 0; i < cnt; i++) {
    if (sff8636_flag_formats[i].fieldName == fieldName) {
...
static bf_qsfp_flag_format_t *find_cmis_flag_format(Sff_field fieldName) {
  int i, cnt = sizeof(cmis_flag_formats) / sizeof(bf_qsfp_flag_format_t);
 ...
 */
static int extract_qsfp_flags(Sff_field fieldName,
                              int port,
                              const uint8_t *data,
                              int chnum,
  ...
  if (is_cmis) {
    ...
    flag_format = find_sff8636_flag_format(fieldName);
  }
  ...
  if (flag_format->is_ch_flag) {  // channel flags
    if (is_cmis) {
      ...
      flags->warn.low = (data[flag_format->low_warn_loc] >> chnum) & 0x1;
      flags->warn.high = (data[flag_format->high_warn_loc] >> chnum) & 0x1;
      flags->alarm.low = (data[flag_format->low_alarm_loc] >> chnum) & 0x1;
      flags->alarm.high = (data[flag_format->high_alarm_loc] >> chnum) & 0x1;
    } else {  // sff-8636
      // loc variables are bit locations
      flags->warn.low = (data[chnum >> 1] >> (flag_format->low_warn_loc +
                                              ((1 - (chnum & 0x1)) * 4))) &
                        0x1;
      flags->warn.high = (data[chnum >> 1] >> (flag_format->high_warn_loc +
                                               ((1 - (chnum & 0x1)) * 4))) &
                         0x1;
      flags->alarm.low = (data[chnum >> 1] >> (flag_format->low_alarm_loc +
                                               ((1 - (chnum & 0x1)) * 4))) &
                         0x1;
      flags->alarm.high = (data[chnum >> 1] >> (flag_format->high_alarm_loc +
                                                ((1 - (chnum & 0x1)) * 4))) &
                          0x1;
    }
  } else {  // module flags
    flags->warn.low = (*data >> flag_format->low_warn_loc) & 0x1;
    flags->warn.high = (*data >> flag_format->high_warn_loc) & 0x1;
    flags->alarm.low = (*data >> flag_format->low_alarm_loc) & 0x1;
    flags->alarm.high = (*data >> flag_format->high_alarm_loc) & 0x1;
  }
  return 0;
}
...
static bool bf_qsfp_read_cache(int port,
  ...
  if (info->page == QSFP_PAGE0_LOWER) {
    cache_loc = &bf_qsfp_info_arr[port].idprom[0 + info->offset];
  } else {
    if (info->page == QSFP_PAGE0_UPPER) {
      cache_loc =
          &bf_qsfp_info_arr[port].page0[info->offset - MAX_QSFP_PAGE_SIZE];
    } else if (!bf_qsfp_is_flat_mem(port)) {
      switch (info->page) {
        case QSFP_PAGE1:
          cache_loc =
              &bf_qsfp_info_arr[port].page1[info->offset - MAX_QSFP_PAGE_SIZE];
          break;
        case QSFP_PAGE2:
          cache_loc =
              &bf_qsfp_info_arr[port].page2[info->offset - MAX_QSFP_PAGE_SIZE];
          break;
        case QSFP_PAGE3:
          cache_loc =
              &bf_qsfp_info_arr[port].page3[info->offset - MAX_QSFP_PAGE_SIZE];
          break;
        default:
...
static int decode_bank(int port,
  ...
  int shifted_chmask;
  if (bf_qsfp_is_cmis(port)) {
    if (info->bank == QSFP_BANKNA) {  // no bank, so bank number is 0
      *decoded_bank = 0;
    } else if (info->bank == QSFP_BANKCH) {  // bank is based on channel number
      ...
      *decoded_bank = 0;
      ...
      if (chmask != (shifted_chmask << (*decoded_bank * 8))) {
        LOG_ERROR(
...
static int decode_page(int port, sff_field_info_t *info, int *decoded_page) {
  if (info->page == QSFP_PAGE0_LOWER) {
    *decoded_page = 0;
  } else {
...
int bf_qsfp_field_read_onebank(int port,
  ...
  if (get_sff_field_info(port, field, &info) < 0) {
    LOG_ERROR("QSFP    %2d : field %d not decodable", port, field);
    return -1;
  }

  if (!info) {
    LOG_ERROR("QSFP    %2d : field %d decoded to NULL", port, field);
    return -1;
  }
  ...
  if (!bf_qsfp_read_cache(port, info, data)) {
    ...
    // read that location
    if (bf_qsfp_module_read(port, bank, page, offset, info->length, data) < 0) {
      if (!bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs) {
        LOG_ERROR("qsfp %d read failed\n", port);
      }
      return -1;
    }
  }
...
static int bf_qsfp_field_write_onebank(int port,
  ...
  // write that location
  if (bf_qsfp_module_write(port, bank, page, offset, info->length, data) < 0) {
...
static int bf_qsfp_bitfield_rmw_onebank(int port,
  ...
  if (chmask != 0xFF) {
    // read
    if (bf_qsfp_field_read_onebank(port, field, host_chmask, 0, 1, &buf) < 0) {
      return -1;
    }
  ...
  // write
  if (bf_qsfp_field_write_onebank(port, field, host_chmask, 0, 1, &buf) < 0) {
    return -1;
  }
  return 0;
}
...
int bf_qsfp_bitfield_rmw(
    int port, Sff_field field, uint host_chmask, uint chmask, uint newdata) {
  uint8_t current_bank = 0;

  if ((port > bf_plt_max_qsfp) ||
      (host_chmask >= (1UL << bf_qsfp_info_arr[port].num_ch))) {
    return -1;
  }
...
int bf_qsfp_module_read(
  ...
  bf_sys_mutex_lock(&bf_qsfp_info_arr[port].qsfp_mtx);
  // safe override
  if (bank == QSFP_BANKNA) {
    bank = 0;
  }
  ...
  if (bf_qsfp_vec.pltfm_read) {
    rc = bf_qsfp_vec.pltfm_read(port, bank, page, offset, len, buf, 0, NULL);
    bf_sys_mutex_unlock(&bf_qsfp_info_arr[port].qsfp_mtx);
    return rc;
  }

  // For Tofino based system compatibility
  if (!bf_qsfp_is_flat_mem(port) && (offset >= MAX_QSFP_PAGE_SIZE)) {
    uint8_t pg = (uint8_t)page;
    rc = bf_pltfm_qsfp_write_module(port, 127, 1, &pg);
  }

  rc |= (bf_pltfm_qsfp_read_module(port, offset, len, buf));
  bf_sys_mutex_unlock(&bf_qsfp_info_arr[port].qsfp_mtx);
  return rc;
}
...
int bf_qsfp_module_write(
  ...
  if (bf_qsfp_vec.pltfm_write) {
    rc = bf_qsfp_vec.pltfm_write(port, bank, page, offset, len, buf, 0, NULL);
    bf_sys_mutex_unlock(&bf_qsfp_info_arr[port].qsfp_mtx);
    return rc;
  }
  ...
  rc |= (bf_pltfm_qsfp_write_module(port, offset, len, buf));
  bf_sys_mutex_unlock(&bf_qsfp_info_arr[port].qsfp_mtx);
  return rc;
}
...
int bf_qsfp_refresh_flags(int port) {
  int res, chmask, cur_chmask, loopcnt, loopcnt2, bitcnt;
  ...
  res = bf_qsfp_field_read_onebank(
      port, MODULE_FLAGS, 0, 0, MODULE_FLAG_BYTES, new_module_flags);
  if (res != 0) {
    return res;
  }
  for (loopcnt2 = 0; loopcnt2 < MODULE_FLAG_BYTES; loopcnt2++) {
    // increment flag counters for all set flags
    for (bitcnt = 0; bitcnt < 8; bitcnt++) {
      if ((new_module_flags[loopcnt2] >> bitcnt) & 0x1) {
        bf_qsfp_info_arr[port].module_flag_cnt[loopcnt2][bitcnt]++;
      }
    }

    bf_qsfp_info_arr[port].module_flags[loopcnt2] |= new_module_flags[loopcnt2];
  }
  ...
  if (bf_qsfp_is_cmis(port)) {
    ...
    while (chmask & cur_chmask) {
      ...
      for (loopcnt2 = 0; loopcnt2 < CMIS_LANE_FLAG_BYTES; loopcnt2++) {
        // increment flag counters for all set flags
        for (bitcnt = 0; bitcnt < 8; bitcnt++) {
        ...
        bf_qsfp_info_arr[port]
            .lane_flags[(loopcnt * CMIS_LANE_FLAG_BYTES) + loopcnt2] |=
    ...
    for (loopcnt2 = 0; loopcnt2 < SFF8636_LANE_STATUS_FLAG_BYTES; loopcnt2++) {
      // increment flag counters for all set flags
      for (bitcnt = 0; bitcnt < 8; bitcnt++) {
        if ((new_lane_flags[loopcnt2] >> bitcnt) & 0x1) {
          bf_qsfp_info_arr[port].lane_flag_cnt[loopcnt2][bitcnt]++;
      ...
      bf_qsfp_info_arr[port].lane_flags[loopcnt2] |= new_lane_flags[loopcnt2];
    }
    res = bf_qsfp_field_read_onebank(port,
                                     SFF8636_LANE_MONITOR_FLAGS,
                                     0,
    ...
    for (loopcnt2 = 0; loopcnt2 < SFF8636_LANE_MONITOR_FLAGS; loopcnt2++) {
      // increment flag counters for all set flags
      for (bitcnt = 0; bitcnt < 8; bitcnt++) {
        if ((new_lane_flags[loopcnt2] >> bitcnt) & 0x1) {
          bf_qsfp_info_arr[port].lane_flag_cnt[loopcnt2][bitcnt]++;
      ...
      bf_qsfp_info_arr[port]
          .lane_flags[SFF8636_LANE_STATUS_FLAG_BYTES + loopcnt2] |=
          new_lane_flags[loopcnt2];
    }
  }
  return res;
...
static sff_flag_map_t *get_sff_flag_addr(const Sff_flag flag) {
  int i, cnt = sizeof(sff8636_flag_map) / sizeof(sff_flag_map_t);
...
static sff_flag_map_t *get_cmis_flag_addr(const Sff_flag flag) {
  int i, cnt = sizeof(cmis_flag_map) / sizeof(sff_flag_map_t);
...
static int get_sff_flag_info(int port, Sff_flag flag, sff_flag_map_t **info) {
 ...
 */
static int convert_flag_info_to_array_ind(int port,
  ...
  if (bf_qsfp_is_cmis(port)) {
    flag_field_info = get_cmis_field_addr(flag_info->field);
  } else {  // sff-8636
    flag_field_info = get_sff_field_addr(flag_info->field);
  }
  ...
  if (flag_info->field == SFF8636_LANE_MONITOR_FLAGS) {
  ...
  *array_ind = flag_info->firstbyte - offset +
               ...
               (((ln_num & 0x7) * flag_info->laneincr) >> 3);  // add ln offset
...
int bf_qsfp_clear_flag(int port, Sff_flag flag, int ln, int bit, bool clrcnt) {
  ...
  if (get_sff_flag_info(port, flag, &flag_info) < 0) {
    LOG_ERROR("QSFP    %2d : flag %d not decodable", port, flag);
    return -1;
  }

  if (!flag_info) {
    LOG_ERROR("QSFP    %2d : flag %d decoded to NULL", port, flag);
    return -1;
  }
  ...
  while (cur_bit < flag_info->numbits) {
    if (bitmask & (1 << cur_bit)) {
      if (flag_info->perlaneflag) {
        ...
        while (cur_ln < bf_qsfp_info_arr[port].num_ch) {
          if (chmask & (1 << cur_ln)) {
            if (convert_flag_info_to_array_ind(
                    port, flag_info, cur_ln, cur_bit, &array_ind, &bit_ind) !=
                0) {
              return -1;
            }
            // clear the flag bit
            bf_qsfp_info_arr[port].lane_flags[array_ind] &= ~(1 << bit_ind);
            if (clrcnt) {
              bf_qsfp_info_arr[port].lane_flag_cnt[array_ind][bit_ind] = 0;
            }
          }
          cur_ln++;
        }
        ...
        if (convert_flag_info_to_array_ind(
                port, flag_info, 0, cur_bit, &array_ind, &bit_ind) != 0) {
          return -1;
        }
        bf_qsfp_info_arr[port].module_flags[array_ind] &= ~(1 << bit_ind);
        if (clrcnt) {
          bf_qsfp_info_arr[port].module_flag_cnt[array_ind][bit_ind] = 0;
        }
      }
    }
    cur_bit++;
...
int bf_qsfp_get_flag(int port, Sff_flag flag, int ln, int bit, uint32_t *cnt) {
  ...
  if (convert_flag_info_to_array_ind(
          port, flag_info, ln, bit, &array_ind, &bit_ind) != 0) {
    return -1;
  }

  if (flag_info->perlaneflag) {
    result = (bf_qsfp_info_arr[port].lane_flags[array_ind] >> bit_ind) & 0x1;
    if (cnt != NULL) {
      *cnt = bf_qsfp_info_arr[port].lane_flag_cnt[array_ind][bit_ind];
    }
  } else {
    result = (bf_qsfp_info_arr[port].module_flags[array_ind] >> bit_ind) & 0x1;
    if (cnt != NULL) {
...
static int get_qsfp_module_sensor_flags(int port,
  ...
  if (bf_qsfp_field_read_onebank(port, fieldName, 0, .0, 1, &data) < 0) {
    return -1;
  }
  return extract_qsfp_flags(fieldName, port, &data, 0, flags);
}
...
static double get_qsfp_module_sensor(int port,
  ...
  if (bf_qsfp_field_read_onebank(port, fieldName, 0, .0, 2, data) < 0) {
    return 0.0;
  }
...
static void get_qsfp_str(int port,
  ...
  if (bf_qsfp_field_read_onebank(
          port, field, 0, 0, buf_size - 1, (uint8_t *)buf) < 0) {
    return;
  }
...
static float get_qsfp_cable_length(int port, Sff_field fieldName) {
  ...
  if ((bf_qsfp_is_cmis(port)) && (fieldName == LENGTH_CBLASSY)) {
...
static int get_threshold_values(int port,
  ...
  if (bf_qsfp_field_read_onebank(port, field, 0, 0, 8, data) < 0) {
    return -1;
  }

  thresh->alarm.high = con_fn((data[0] << 8) | data[1]);
  thresh->alarm.low = con_fn((data[2] << 8) | data[3]);
  thresh->warn.high = con_fn((data[4] << 8) | data[5]);
  thresh->warn.low = con_fn((data[6] << 8) | data[7]);
...
int bf_qsfp_get_module_type(int port, uint8_t *module_type) {
  return bf_qsfp_field_read_onebank(port, IDENTIFIER, 0, 0, 1, module_type);
}
...
bool bf_qsfp_get_module_type_str(int port,
  ...
  if (bf_qsfp_get_module_type(port, &module_type) >= 0) {
    switch (module_type) {
      case QSFP:
...
bool bf_qsfp_get_vendor_info(int port, qsfp_vendor_info_t *vendor) {
  get_qsfp_str(port, VENDOR_NAME, vendor->name, sizeof(vendor->name));
  bf_qsfp_field_read_onebank(port, VENDOR_OUI, 0, 0, 3, vendor->oui);
  get_qsfp_str(
      port, PART_NUMBER, vendor->part_number, sizeof(vendor->part_number));
  get_qsfp_str(port, REVISION_NUMBER, vendor->rev, sizeof(vendor->rev));
  get_qsfp_str(port,
               VENDOR_SERIAL_NUMBER,
               vendor->serial_number,
               sizeof(vendor->serial_number));
  get_qsfp_str(port, MFG_DATE, vendor->date_code, sizeof(vendor->date_code));
  return true;
}
...
bool bf_qsfp_get_threshold_info(int port, qsfp_alarm_threshold_t *thresh) {
  ...
  rc = get_threshold_values(port, TEMPERATURE_THRESH, &thresh->temp, get_temp);
  rc |= get_threshold_values(port, VCC_THRESH, &thresh->vcc, get_vcc);
  rc |= get_threshold_values(port, RX_PWR_THRESH, &thresh->rx_pwr, get_pwr);
  rc |=
      get_threshold_values(port, TX_BIAS_THRESH, &thresh->tx_bias, get_txbias);
  rc |= get_threshold_values(port, TX_PWR_THRESH, &thresh->tx_pwr, get_pwr);
  return (rc ? false : true);
}

int bf_qsfp_get_threshold_temp(int port, qfsp_threshold_level_t *thresh) {
  return get_threshold_values(port, TEMPERATURE_THRESH, thresh, get_temp);
}

int bf_qsfp_get_threshold_vcc(int port, qfsp_threshold_level_t *thresh) {
  return get_threshold_values(port, VCC_THRESH, thresh, get_vcc);
}

int bf_qsfp_get_threshold_rx_pwr(int port, qfsp_threshold_level_t *thresh) {
  return get_threshold_values(port, RX_PWR_THRESH, thresh, get_pwr);
}

int bf_qsfp_get_threshold_tx_bias(int port, qfsp_threshold_level_t *thresh) {
  return get_threshold_values(port, TX_BIAS_THRESH, thresh, get_txbias);
}
...
bool bf_qsfp_get_module_sensor_info(int port, qsfp_global_sensor_t *info) {
  info->temp.value = get_qsfp_module_sensor(port, TEMPERATURE, get_temp);
  info->temp._isset.value = true;
  get_qsfp_module_sensor_flags(port, TEMPERATURE_ALARMS, &(info->temp.flags));
  info->temp._isset.flags = true;
  info->vcc.value = get_qsfp_module_sensor(port, VCC, get_vcc);
  info->vcc._isset.value = true;
  get_qsfp_module_sensor_flags(port, VCC_ALARMS, &(info->vcc.flags));
...
double bf_qsfp_get_temp_sensor(int port) {
  return get_qsfp_module_sensor(port, TEMPERATURE, get_temp);
}
...
bool bf_qsfp_get_cable_info(int port, qsfp_cable_t *cable) {
  ...
  cable->is_cable_assy = bf_qsfp_is_cable_assy(port);

  if ((!bf_qsfp_is_passive_cu(port)) && (!cable->is_cable_assy)) {
    cable->xcvr_single_mode =
        (int32_t)get_qsfp_cable_length(port, LENGTH_SM_KM);
    cable->_isset.xcvr_single_mode = (cable->xcvr_single_mode != 0);
    if (bf_qsfp_is_cmis(port)) {
      cable->xcvr_om5 = (int32_t)get_qsfp_cable_length(port, LENGTH_OM5);
      cable->_isset.xcvr_om5 = (cable->xcvr_om5 != 0);
      cable->xcvr_om4 = (int32_t)get_qsfp_cable_length(port, LENGTH_OM4);
      cable->_isset.xcvr_om4 = (cable->xcvr_om4 != 0);
    }
    cable->xcvr_om3 = (int32_t)get_qsfp_cable_length(port, LENGTH_OM3);
    cable->_isset.xcvr_om3 = (cable->xcvr_om3 != 0);
    cable->xcvr_om2 = (int32_t)get_qsfp_cable_length(port, LENGTH_OM2);
    cable->_isset.xcvr_om2 = (cable->xcvr_om2 != 0);
    if (!bf_qsfp_is_cmis(port)) {
      cable->xcvr_om1 = (int32_t)get_qsfp_cable_length(port, LENGTH_OM1);
      cable->_isset.xcvr_om1 = (cable->xcvr_om1 != 0);
    }
  } else {
    cable->cable_assy = get_qsfp_cable_length(port, LENGTH_CBLASSY);
    cable->_isset.cable_assy = true;  // 0m means < 1m in some cases
  }
  return (cable->_isset.cable_assy || cable->_isset.xcvr_om1 ||
          cable->_isset.xcvr_om2 || cable->_isset.xcvr_om3 ||
          cable->_isset.xcvr_om4 || cable->_isset.xcvr_om5 ||
...
bool bf_qsfp_get_module_state_info(
    int port, qsfp_module_state_info_t *module_state_info) {
  uint8_t data;
  if (!bf_qsfp_is_cmis(port)) {
    return false;
  }

  if (bf_qsfp_field_read_onebank(port, MODULE_STATE, 0, 0, 1, &data) < 0) {
    return false;
  }
  module_state_info->module_state = (data >> 1) & 0x7;
  switch (module_state_info->module_state) {
    case MODULE_ST_LOWPWR:
      strcpy(module_state_info->module_state_str, "ModuleLowPwr");
      break;
    case MODULE_ST_PWRUP:
      strcpy(module_state_info->module_state_str, "ModulePwrUp");
      break;
    case MODULE_ST_READY:
      strcpy(module_state_info->module_state_str, "ModuleReady");
      break;
    case MODULE_ST_PWRDN:
      strcpy(module_state_info->module_state_str, "ModulePwrDn");
      break;
    case MODULE_ST_FAULT:
      strcpy(module_state_info->module_state_str, "Fault");
      break;
    default:
      strcpy(module_state_info->module_state_str, "Unknown");
      break;
  }

  if (bf_qsfp_field_read_onebank(port, POWER_CONTROL, 0, 0, 1, &data) < 0) {
    return false;
  }
  module_state_info->forceLowPwr = (data >> 4) & 0x1;
...
bool bf_qsfp_get_dp_state_info(int port,
  ...
  for (cur_ch = 0; cur_ch < num_ch; cur_ch += 8) {
    if (bf_qsfp_field_read_onebank(
            port, DATAPATH_STATE_ALL, (chmask & curbank_mask), 0, 4, dp_state) <
        0) {
      return false;
    }
    if (bf_qsfp_field_read_onebank(
            port, DATAPATH_DEINIT, (chmask & curbank_mask), 0, 1, &dp_deinit) <
        0) {
      return false;
    }
    for (sub_ch = cur_ch; ((sub_ch < num_ch) && (sub_ch < cur_ch + 8));
         sub_ch++) {
      dp_state_info[sub_ch].data_path_deinit_bit =
          (dp_deinit >> (sub_ch % 8)) & 0x1;
      dp_state_info[sub_ch].datapath_state =
          (dp_state[sub_ch >> 1] >> (4 * (sub_ch & 0x1))) & 0xF;
      switch (dp_state_info[sub_ch].datapath_state) {
        case DATAPATH_ST_DEACTIVATED:
          strcpy(dp_state_info[sub_ch].datapath_state_str,
                 "DataPathDeactivated");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "DAct");
          break;
        case DATAPATH_ST_INIT:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "DataPathInit");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "Init");
          break;
        case DATAPATH_ST_DEINIT:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "DataPathDeinit");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "DInt");
          break;
        case DATAPATH_ST_ACTIVATED:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "DataPathActivated");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "Act");
          break;
        case DATAPATH_ST_TXTURNON:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "DataPathTxTurnOn");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "TOn");
          break;
        case DATAPATH_ST_TXTURNOFF:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "DataPathTxTurnOff");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "TOff");
          break;
        case DATAPATH_ST_INITIALIZED:
          strcpy(dp_state_info[sub_ch].datapath_state_str,
                 "DataPathInitialized");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "Ized");
          break;
        default:
          strcpy(dp_state_info[sub_ch].datapath_state_str, "Unknown");
          strcpy(dp_state_info[sub_ch].datapath_state_str_short, "Unk");
          break;
      }
    }
    curbank_mask <<= 8;
...
bool bf_qsfp_get_dp_state_config(int port,
  ...
  for (cur_ch = 0; cur_ch < num_ch; cur_ch += 8) {
    if (bf_qsfp_field_read_onebank(
            port, field_name, (chmask & curbank), 0, sizeof(data), data) < 0) {
      return false;
    }
    for (sub_ch = cur_ch; ((sub_ch < num_ch) && (sub_ch < cur_ch + 8));
         sub_ch++) {
      set_cfg[sub_ch].selected_ApSel_code = data[sub_ch % 8] >> 4;
      set_cfg[sub_ch].data_path_id = (data[sub_ch % 8] >> 1) & 0x7;
      set_cfg[sub_ch].explicit_control = data[sub_ch % 8] & 0x1;
    }
    curbank <<= 8;
  }
  return true;
...
bool bf_qsfp_get_chan_sensor_data(int port, qsfp_channel_t *chn) {
  if (!bf_qsfp_get_chan_tx_bias(port, chn)) {
    LOG_ERROR("%s: bf_qsfp_get_chan_tx_bias(port=%d) failed", __func__, port);
    return false;
  }

  if (!bf_qsfp_get_chan_tx_bias_alarm(port, chn)) {
    LOG_ERROR(
        "%s: bf_qsfp_get_chan_tx_bias_alarm(port=%d) failed", __func__, port);
    return false;
  }

  if (!bf_qsfp_get_chan_tx_pwr(port, chn)) {
    LOG_ERROR("%s: bf_qsfp_get_chan_tx_pwr(port=%d) failed", __func__, port);
    return false;
  }

  if (!bf_qsfp_get_chan_tx_pwr_alarm(port, chn)) {
    LOG_ERROR(
        "%s: bf_qsfp_get_chan_tx_pwr_alarm(port=%d) failed", __func__, port);
    return false;
  }

  if (!bf_qsfp_get_chan_rx_pwr(port, chn)) {
    LOG_ERROR("%s: bf_qsfp_get_chan_rx_pwr(port=%d) failed", __func__, port);
    return false;
  }

  if (!bf_qsfp_get_chan_rx_pwr_alarm(port, chn)) {
    LOG_ERROR(
        "%s: bf_qsfp_get_chan_rx_pwr_alarm(port=%d) failed", __func__, port);
    return false;
  }
...
bool bf_qsfp_get_chan_tx_bias(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  ...
  if (bf_qsfp_field_read_onebank(port, CHANNEL_TX_BIAS, chmask, 0, 16, data) <
      0) {
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
...
bool bf_qsfp_get_chan_tx_pwr(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  int chmask = bf_qsfp_chmask_get(port);
  ...
  if (bf_qsfp_field_read_onebank(port, CHANNEL_TX_PWR, chmask, 0, 16, data) <
      0) {
    LOG_ERROR("%s: bf_qsfp_field_read_onebank(port=%d,chmask=%02x) failed",
              __func__,
              port,
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
    uint16_t val = data[i * 2] << 8 | data[(i * 2) + 1];
    chn[i].sensors.tx_pwr.value = get_pwr(val);
...
bool bf_qsfp_get_chan_rx_pwr(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  int chmask = bf_qsfp_chmask_get(port);
  ...
  if (bf_qsfp_field_read_onebank(port, CHANNEL_RX_PWR, chmask, 0, 16, data) <
      0) {
    LOG_ERROR("%s: bf_qsfp_field_read_onebank(port=%d,chmask=%02x) failed",
              __func__,
              port,
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
    uint16_t val = data[i * 2] << 8 | data[(i * 2) + 1];
    chn[i].sensors.rx_pwr.value = get_pwr(val);
...
bool bf_qsfp_get_chan_tx_bias_alarm(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  int chmask = bf_qsfp_chmask_get(port);
  ...
  if (bf_qsfp_field_read_onebank(
          port, CHANNEL_TX_BIAS_ALARMS, chmask, 0, 16, data) < 0) {
    LOG_ERROR("%s: bf_qsfp_field_read_onebank(port=%d,chmask=%02x) failed",
              __func__,
              port,
              chmask);
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
    extract_qsfp_flags(
        CHANNEL_TX_BIAS_ALARMS, port, data, i, &chn[i].sensors.tx_bias.flags);
...
bool bf_qsfp_get_chan_tx_pwr_alarm(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  int chmask = bf_qsfp_chmask_get(port);
  ...
  if (bf_qsfp_field_read_onebank(
          port, CHANNEL_TX_PWR_ALARMS, chmask, 0, 16, data) < 0) {
    LOG_ERROR("%s: bf_qsfp_field_read_onebank(port=%d,chmask=%02x) failed",
              __func__,
              port,
              chmask);
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
    extract_qsfp_flags(
        CHANNEL_TX_PWR_ALARMS, port, data, i, &chn[i].sensors.tx_pwr.flags);
...
bool bf_qsfp_get_chan_rx_pwr_alarm(int port, qsfp_channel_t *chn) {
  uint8_t data[16];
  int i;
  int chmask = bf_qsfp_chmask_get(port);
  ...
  if (bf_qsfp_field_read_onebank(
          port, CHANNEL_RX_PWR_ALARMS, chmask, 0, 16, data) < 0) {
    LOG_ERROR("%s: bf_qsfp_field_read_onebank(port=%d,chmask=%02x) failed",
              __func__,
              port,
              chmask);
  ...
  for (i = 0; i < bf_qsfp_info_arr[port].num_ch && i < MAX_CHAN_PER_CONNECTOR;
       i++) {
    extract_qsfp_flags(
        CHANNEL_RX_PWR_ALARMS, port, data, i, &chn[i].sensors.rx_pwr.flags);
...
int bf_qsfp_get_flag_for_all_chans(int port, Sff_flag flag, int *flag_val) {
  int n, v;

  for (n = 0; n < bf_qsfp_info_arr[port].num_ch && n < MAX_CHAN_PER_CONNECTOR;
       ++n) {
...
double bf_qsfp_get_voltage(int port) {
  return get_qsfp_module_sensor(port, VCC, get_vcc);
}
...
bool bf_qsfp_get_firmware_ver(int port,
  ...
  uint8_t active_ver_data[2] = {0}, inactive_ver_data[2] = {0};
  *active_ver_major = 0;
  *active_ver_minor = 0;
  ...
  *inactive_ver_minor = 0;
  ...
  if (bf_qsfp_info_arr[port].memmap_format == MMFORMAT_CMIS3P0) {
    ...
    if (bf_qsfp_field_read_onebank(
            port, FIRMWARE_VER_INACTIVE, 0, 0, 2, active_ver_data) < 0) {
      return false;
    }
  } else {  // 4.0 and later
    if (bf_qsfp_field_read_onebank(
            port, FIRMWARE_VER_ACTIVE, 0, 0, 2, active_ver_data) < 0) {
      return false;
    }

    if (bf_qsfp_field_read_onebank(
            port, FIRMWARE_VER_INACTIVE, 0, 0, 2, inactive_ver_data) < 0) {
      return false;
    }
  }
...
bool bf_qsfp_get_hardware_ver(int port,
  ...
  *hw_ver_minor = 0;
  ...
  if ((!bf_qsfp_is_cmis(port)) || (bf_qsfp_is_flat_mem(port))) {
    return false;  // sff-8636 does not currently support
  }

  if (bf_qsfp_field_read_onebank(port, HARDWARE_VER, 0, 0, 2, hw_ver_data) <
      0) {
    return false;
  }
...
static void bf_qsfp_idprom_clr(int port) {
  ...
  memset(&(bf_qsfp_info_arr[port].idprom[0]), 0, MAX_QSFP_PAGE_SIZE);
  memset(&(bf_qsfp_info_arr[port].page0[0]), 0, MAX_QSFP_PAGE_SIZE);
  memset(&(bf_qsfp_info_arr[port].page1[0]), 0, MAX_QSFP_PAGE_SIZE);
  memset(&(bf_qsfp_info_arr[port].page2[0]), 0, MAX_QSFP_PAGE_SIZE);
  memset(&(bf_qsfp_info_arr[port].page3[0]), 0, MAX_QSFP_PAGE_SIZE);
 ...
 *  @param num_ports
 */
int bf_qsfp_get_max_qsfp_ports(void) { return bf_plt_max_qsfp; }
 ...
 */
bool bf_qsfp_has_pages(int port) { return !bf_qsfp_info_arr[port].flat_mem; }
...
int bf_qsfp_tx_disable_single_lane(int port, int channel, bool disable) {
  ...
  rc = bf_qsfp_bitfield_rmw_onebank(
      port, CHANNEL_TX_DISABLE, (1 << channel), (1 << channel), data);
...
int bf_qsfp_tx_disable(int port, int channel_mask, bool disable) {
  ...
  rc = bf_qsfp_bitfield_rmw(
      port, CHANNEL_TX_DISABLE, channel_mask, channel_mask, new_st);
  bf_sys_usleep(400000);
...
int bf_qsfp_dp_deactivate_all(int port) {
  ...
  uint8_t newval;
  if ((port > bf_plt_max_qsfp) || (!bf_qsfp_is_cmis(port))) {
    return -1;
  }
  ...
  all_chmask = (1 << bf_qsfp_info_arr[port].num_ch) - 1;
  curbank_chmask = 0xFF;
...
int bf_cmis_spec_rev_get(int port, uint8_t *major, uint8_t *minor) {
  uint8_t rev;
  if (bf_qsfp_field_read_onebank(port, SPEC_REV, 0, 0, 1, &rev) < 0) {
    return -1;
  }
  if (major != NULL) {
    *major = rev >> 4;
...
static int set_qsfp_idprom(int port) {
  ...
  if (bf_qsfp_field_read_onebank(port, IDENTIFIER, 0, 0, 1, &id) < 0) {
    if (!bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs) {
  ...
  if ((id == QSFP) || (id == QSFP_PLUS) || (id == QSFP_28)) {
    bf_qsfp_info_arr[port].memmap_format = MMFORMAT_SFF8636;
    bf_qsfp_info_arr[port].num_ch = 4;
  } else {  // CMIS
    // now determine if the CMIS memory map is rev 3.0 or 4.0+
    if (bf_cmis_spec_rev_get(port, &major_rev, NULL) < 0) {
      LOG_ERROR("Unable to obtain revision field in module %d\n", port);
      return -1;
    }
    if (major_rev < 4) {  // CMIS 3.X
      bf_qsfp_info_arr[port].memmap_format = MMFORMAT_CMIS3P0;
    } else if (major_rev < 5) {  // CMIS 4.0 or newer. There was a break in
                                 // backwards compatibility
      bf_qsfp_info_arr[port].memmap_format = MMFORMAT_CMIS4P0;
    } else {
      bf_qsfp_info_arr[port].memmap_format = MMFORMAT_CMIS5P0;
    }
  ...
  // paged memory
  if (bf_qsfp_field_read_onebank(port, STATUS, 0, 0, 1, &status) < 0) {
    LOG_ERROR("Unable to obtain status field in module %d\n", port);
    return -1;
  }
  if (bf_qsfp_info_arr[port].memmap_format == MMFORMAT_SFF8636) {
    bf_qsfp_info_arr[port].flat_mem = (status >> 2) & 1;
  } else {  // CMIS and UNKNOWN
    bf_qsfp_info_arr[port].flat_mem = (status >> 7) & 1;
  }
  return 0;
}
...
void bf_qsfp_get_transceiver_info(int port, qsfp_transciever_info_t *info) {
  ...
  if (bf_qsfp_get_vendor_info(port, &info->vendor)) {
  ...
  if (!bf_qsfp_is_passive_cu(port)) {
    if (bf_qsfp_get_module_sensor_info(port, &info->sensor)) {
      info->_isset.sensor = true;
    }
    if (bf_qsfp_get_threshold_info(port, &info->thresh)) {
      info->_isset.thresh = true;
    }
    for (int i = 0; i < bf_qsfp_info_arr[port].num_ch; i++) {
      info->chn[i].chn = i;
    }

    if (bf_qsfp_get_chan_sensor_data(port, info->chn)) {
...
static int cmis_get_media_type(int port, uint8_t *media_type) {
  ...
  if (bf_qsfp_field_read_onebank(port, MODULE_MEDIA_TYPE, 0, 0, 1, media_type) <
      0) {
    return -1;
  }
  return 0;
...
static int qsfp_cmis_populate_app_list(int port) {
  int cur_app_num = 0;
  while (cur_app_num < bf_qsfp_info_arr[port].num_apps) {
...
static int qsfp_sff8636_add_app(int port,
  ...
  if (bf_qsfp_info_arr[port].num_apps == 1) {
    bf_qsfp_info_arr[port].app_list =
        (bf_pm_qsfp_apps_t *)bf_sys_malloc(sizeof(bf_pm_qsfp_apps_t));
  } else {
    bf_qsfp_info_arr[port].app_list = (bf_pm_qsfp_apps_t *)bf_sys_realloc(
        bf_qsfp_info_arr[port].app_list,
        bf_qsfp_info_arr[port].num_apps * sizeof(bf_pm_qsfp_apps_t));
  }
  ...
  bf_qsfp_info_arr[port].app_list[cur_app].host_if_id = host_if_id;
  bf_qsfp_info_arr[port].app_list[cur_app].media_if_id = media_if_id;
  bf_qsfp_info_arr[port].app_list[cur_app].host_lane_cnt = host_lane_cnt;
  bf_qsfp_info_arr[port].app_list[cur_app].media_lane_cnt = media_lane_cnt;
...
static int qsfp_sff8636_populate_app_list(int port) {
  ...
  if (bf_qsfp_get_eth_ext_compliance(port, &eth_ext_comp) != 0) {
    LOG_ERROR(
  ...
  // Get the ethernet extended compliance type of the cable
  if (bf_qsfp_get_eth_secondary_compliance(port, &eth_sec_comp) != 0) {
    LOG_ERROR(
        "Error : in getting the ethernet extended compliance type of the "
        "QSFP at "
        "%s:%d\n",
  ...
  if (eth_comp & 0x1) {  // 40G active cable (XLPPI)
    qsfp_sff8636_add_app(port, 0x4, 7, 1, 4, 4, 0x1, 0x1);
  } else if ((eth_comp >> 1) & 0x1) {  // 40GBASE-LR4
    qsfp_sff8636_add_app(port, 0x2, 7, 9, 4, 4, 0x1, 0x1);
  } else if ((eth_comp >> 2) & 0x1) {  // 40GBASE-SR4
    qsfp_sff8636_add_app(port, 0x1, 7, 4, 4, 4, 0x1, 0x1);
  } else if ((eth_comp >> 3) & 0x1) {  // 40GBASE-CR4
    qsfp_sff8636_add_app(port, 0x3, 7, 1, 4, 4, 0x1, 0x1);
  }
  ...
  switch (eth_ext_comp) {
    ...
    case ACC_100G_BER_5:  // 100G ACC (Active Copper Cable) or 25GAUI C2M ACC.
                          // Providing a worst BER of 5 ? 10-5
      qsfp_sff8636_add_app(port, 0x4, 11, 2, 4, 4, 0x1, 0x1);
    ...
    case SR4_100GBASE:  // 100GBASE-SR4 or 25GBASE-SR
      ...
      // read the Pg 0 Byte 113 to see
      // the cable construction
      qsfp_sff8636_add_app(port, 0x1, 5, 3, 1, 1, 0xF, 0xF);
      break;
    case LR4_100GBASE:   // 100GBASE-LR4 or 25GBASE-LR
    case PSM4_100G_SMF:  // 100G PSM4
      qsfp_sff8636_add_app(port, 0x2, 11, 13, 4, 4, 0x1, 0x1);

      // assume it supports breakout, but this may be a bad assumption. Need to
      // read the Pg 0 Byte 113 to see
      // the cable construction
      qsfp_sff8636_add_app(port, 0x2, 5, 7, 1, 1, 0xF, 0xF);
      break;
    case ER4_100GBASE:  // 100GBASE-ER4 or 25GBASE-ER
      qsfp_sff8636_add_app(port, 0x2, 11, 14, 4, 4, 0x1, 0x1);

      // assume it supports breakout, but this may be a bad assumption. Need to
      // read the Pg 0 Byte 113 to see
      // the cable construction
      qsfp_sff8636_add_app(port, 0x2, 5, 8, 1, 1, 0xF, 0xF);
      break;
    case SR10_100GBASE:  // 100GBASE-SR10
      // not in CMIS
      break;
    case CWDM4_100G:  // 100G CWDM4
      qsfp_sff8636_add_app(port, 0x2, 11, 16, 4, 4, 0x1, 0x1);
      break;
    case CR4_100GBASE:  // 100GBASE-CR4 or 25GBASE-CR CA-25G-L
      qsfp_sff8636_add_app(port, 0x3, 26, 1, 4, 4, 0x1, 0x1);

      // assume it supports breakout, but this may be a bad assumption. Need to
      // read the Pg 0 Byte 113 to see
      // the cable construction
      qsfp_sff8636_add_app(port, 0x3, 20, 1, 1, 1, 0xF, 0xF);
      break;
    case CR_25GBASE_CA_S:  // 25GBASE-CR CA-25G-S
      qsfp_sff8636_add_app(port, 0x3, 21, 1, 1, 1, 0xF, 0xF);
      break;
    case CR_25GBASE_CA_N:  // 25GBASE-CR CA-25G-N
      qsfp_sff8636_add_app(port, 0x3, 22, 1, 1, 1, 0xF, 0xF);
      break;
  ...
  if (!bf_qsfp_info_arr[port].num_apps) {
    ...
    if (bf_qsfp_is_optical(port)) {
      ...
      qsfp_sff8636_add_app(port, 0x3, 26, 1, 4, 4, 0x1, 0x1);  // 100G Copper
      // assume it supports breakout, but this may be a bad assumption. Need to
      // read the Pg 0 Byte 113 to see
      // the cable construction
      qsfp_sff8636_add_app(port, 0x3, 20, 1, 1, 1, 0xF, 0xF);
...
static int bf_qsfp_chksum_chk(int port, uint8_t *buf, bool is_cmis) {
  int i = 0;
  uint32_t base_chksum = 0, ext_chksum = 0, page_chksum = 0;
  ...
  if (is_cmis) {
    ...
    if (buf[94] != page_chksum) return -1;
    ...
    if (buf[63] != base_chksum) return -1;
...
static int bf_qsfp_update_cache(int port) {
  ...
  LOG_DEBUG(
      "QSFP    %2d : %s", port, bf_qsfp_is_cmis(port) ? "CMIS" : "SFF-8636");

  LOG_DEBUG(
      "QSFP    %2d : %s", port, bf_qsfp_is_flat_mem(port) ? "Flat" : "Paged");

  LOG_DEBUG(
      "QSFP    %2d : Lane count = %d", port, bf_qsfp_info_arr[port].num_ch);
  ...
  if (bf_qsfp_chksum_chk(
          port, &bf_qsfp_info_arr[port].page0[0], bf_qsfp_is_cmis(port))) {
    if (!bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs) {
  ...
  if (bf_qsfp_is_cmis(port)) {
    ...
    if (cmis_get_media_type(port, &bf_qsfp_info_arr[port].media_type) != 0) {
      LOG_ERROR(
    ...
    if (cmis_calc_application_count(port, &bf_qsfp_info_arr[port].num_apps) !=
        0) {
      LOG_ERROR(
    ...
    bf_qsfp_info_arr[port].app_list = (bf_pm_qsfp_apps_t *)bf_sys_malloc(
        bf_qsfp_info_arr[port].num_apps * sizeof(bf_pm_qsfp_apps_t));
    if (!bf_qsfp_info_arr[port].app_list) {
      LOG_ERROR(
          "Error in malloc for app_list at "
  ...
  LOG_DEBUG("QSFP    %2d : Update cache complete", port);
  return 0;
}
...
int bf_qsfp_detect_transceiver(int port, bool *is_present) {
  ...
  if (st != bf_qsfp_info_arr[port].present ||
      bf_qsfp_info_arr[port].cache_dirty) {
    if (!bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs) {
      LOG_DEBUG("QSFP    %2d : %s", port, st ? "PRESENT" : "REMOVED");
    }
    ...
    *is_present = st;
    if (st) {
      if (bf_qsfp_update_cache(port)) {
         ...
         */
        bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs = true;
        return -1;
      }
      bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs = false;
...
int bf_qsfp_get_transceiver_pres(uint32_t *lower_ports,
  ...
  rc = bf_pltfm_qsfp_get_presence_mask(lower_ports, upper_ports, cpu_ports);

  // If the qsfp is plugged-in and soft-removal is set, or force present is
  // set, override presence
  for (port = 0; port <= (uint32_t)bf_plt_max_qsfp; port++) {
    pos = port % 32;
    if (port < 32) {
...
void bf_qsfp_get_transceiver_int(uint32_t *lower_ports,
                                 uint32_t *upper_ports,
                                 uint32_t *cpu_ports) {
  bf_pltfm_qsfp_get_int_mask(lower_ports, upper_ports, cpu_ports);
}
...
int bf_qsfp_get_transceiver_lpmode(uint32_t *lower_ports,
                                   uint32_t *upper_ports,
                                   uint32_t *cpu_ports) {
  return (bf_pltfm_qsfp_get_lpmode_mask(lower_ports, upper_ports, cpu_ports));
}
 ...
 *   port
 *
 * Returns BF_INVALID_ARG if the port is not valid.
 * Returns BF_HW_COMM_FAIL if setting the power mode actually fails.
 */
bf_status_t bf_qsfp_set_pwr_ctrl(int port, bool lpmode) {
  ...
  if (!bf_qsfp_is_cmis(port)) {  // SFF-8636
    uint8_t extId;
    if (bf_qsfp_field_read_onebank(port, PWR_REQUIREMENTS, 0, 0, 1, &extId) <
        0) {
      LOG_DEBUG("QSFP    %2d : qsfp ext id read failure", port);
      return BF_HW_COMM_FAIL;
    }

    int high_pwr_level = (extId & EXT_ID_HI_POWER_MASK);
    int power_level = (extId & EXT_ID_MASK) >> EXT_ID_SHIFT;
    ...
    if (lpmode) {
      new_pwr_ctrl |= POWER_SET;  // this bit set to 1 forces LPMode
    }
  } else {  // is CMIS
    // CMIS 3.0 and CMIS 4.0+ do this differently
    if (bf_qsfp_info_arr[port].memmap_format == MMFORMAT_CMIS3P0) {
      LOG_DEBUG("QSFP    %2d : CMIS <= v3 Power Control", port);
      if (lpmode) {
  ...
  if (bf_qsfp_field_write_onebank(
    ...
    LOG_ERROR("QSFP    %2d : Error writing Power Control", port);
    return BF_HW_COMM_FAIL;
  }
...
int bf_qsfp_get_info(int port, int page, uint8_t *buf) {
  ...
  if (bf_qsfp_is_optical(port)) {
    ...
    if (page == QSFP_PAGE0_UPPER) {
      if (bf_qsfp_chksum_chk(port, eeprom_info, bf_qsfp_is_cmis(port))) {
        if (!bf_qsfp_info_arr[port].suppress_repeated_rd_fail_msgs) {
...
cache_segment_list_t *find_suitable_segment_list(unsigned int port, int page) {
  static cache_segment_t sff8636_and_cmis_page0_upper_cache[] = {
      {0, 128},
  };
  int page_count = 0;
  ...
  if (bf_qsfp_is_sff8636(port)) {
    ...
    page_count = sizeof(sff8636_cache) / sizeof(cache_segment_list_t);
  } else if (bf_qsfp_is_cmis(port)) {
    ...
    page_count = sizeof(cmis_cache) / sizeof(cache_segment_list_t);
  }
...
cache_segment_t *find_suitable_segment(cache_segment_t *cache,
                                       int cache_size,
                                       int offset) {
  for (int i = 0; i != cache_size; ++i) {
    if (offset >= cache[i].start_offset &&
        offset < (cache[i].start_offset + cache[i].length)) {
      return cache + i;
    }
  }
  return NULL;
 ...
 *   0 on success and -1 on error
 */
bf_pltfm_status_t bf_qsfp_module_cached_read(unsigned int port,
                                             int bank,
                                             int page,
                                             int offset,
                                             int length,
  ...
  while (left_to_read) {
    cache_segment_t *is_cached =
        find_suitable_segment(cache_list, cache_size, curr_offset);
    if (is_cached) {
      int cached_end_offset = is_cached->start_offset + is_cached->length;
      int left_cached = cached_end_offset - curr_offset;
      int cached_length_to_read = min(left_cached, left_to_read);
      ...
      int base_offset = page == QSFP_PAGE0_LOWER ? 0 : 128;
      int device_page = page == QSFP_PAGE0_LOWER ? 0 : page;
...
bool bf_qsfp_tx_is_disabled(int port, int channel) {
  ...
  if ((port > bf_plt_max_qsfp) || (channel >= bf_qsfp_info_arr[port].num_ch)) {
    return true;
  }
  /* read the current value */
  if (bf_qsfp_field_read_onebank(
          port, CHANNEL_TX_DISABLE, (1 << channel), 0, 1, &data) < 0) {
    return true;
  }
...
int bf_qsfp_get_eth_secondary_compliance(int port,
  ...
  if (bf_qsfp_field_read_onebank(
          port, ETHERNET_SECONDARY_COMPLIANCE, 0, 0, 1, &eth_ext_comp) < 0) {
    *code = EXT_COMPLIANCE_NONE;
    return -1;
  }
  *code = eth_ext_comp;
...
bool bf_qsfp_is_active_cbl(int port) {
  ...
  return (bf_qsfp_info_arr[port].media_type == MEDIA_TYPE_ACTIVE_CBL);
}
...
uint8_t bf_qsfp_get_media_ch_cnt(int port) {
  ...
  for (cur_app = 0; cur_app < bf_qsfp_info_arr[port].num_apps; cur_app++) {
    ...
    if (media_ch_cnt <
        max_headch + bf_qsfp_info_arr[port].app_list[cur_app].media_lane_cnt) {
      media_ch_cnt =
          max_headch + bf_qsfp_info_arr[port].app_list[cur_app].media_lane_cnt;
    }
  }

  if (media_ch_cnt == 0) {  // maybe this module doesn't have Apps (e.g. copper)
    media_ch_cnt = bf_qsfp_info_arr[port].num_ch;
  }

  LOG_DEBUG("QSFP    %2d : Media lane count = %d", port, media_ch_cnt);
  return media_ch_cnt;
}

bool bf_qsfp_get_spec_rev_str(int port, char *rev_str) {
  uint8_t rev;
  bf_qsfp_field_read_onebank(port, SPEC_REV, 0, 0, 1, &rev);
  if (bf_qsfp_is_cmis(port)) {
...
int bf_qsfp_special_case_set(int port,
  ...
  if (is_set) {
    bf_qsfp_info_arr[port].special_case_port.qsfp_type = qsfp_type;
    bf_qsfp_info_arr[port].special_case_port.is_set = true;
  }
...
static bool bf_qsfp_get_is_type_copper(bf_pltfm_qsfp_type_t qsfp_type) {
  if (qsfp_type != BF_PLTFM_QSFP_OPT) {
    return true;
  }
  return false;
 ...
 */
static bool bf_qsfp_special_case_handled(int port,
                                         bf_pltfm_qsfp_type_t *qsfp_type,
                                         bool *type_copper) {
  qsfp_vendor_info_t vendor;

  bf_qsfp_get_vendor_info(port, &vendor);
  if (!memcmp(vendor.name, "Ampheno", strlen("Ampheno")) &&
      !memcmp(vendor.part_number, "62162000", strlen("62162000"))) {
    ...
    *type_copper = true;
    ...
    LOG_DEBUG("forcing port %d qsfp type to 3m-copper for %s p/n %s\n",
              ...
              vendor.part_number);
    return true;
  } else if (!memcmp(vendor.name, "ELPEU", strlen("ELPEU")) &&
             !memcmp(
                 vendor.part_number, "QSFP28-LB-6D", strlen("QSFP28-LB-6D"))) {
    ...
    *type_copper = true;
    ...
    LOG_DEBUG("forcing port %d qsfp type to copper-loopback for %s p/n %s\n",
              ...
              vendor.part_number);
    return true;
  } else if (!memcmp(vendor.name, "CISCO-FINISAR", strlen("CISCO-FINISAR")) &&
    ...
    *type_copper = false;
    ...
    return true;
                 ...
                 vendor.name, "Arista Networks", strlen("Arista Networks")) &&
             !memcmp(vendor.part_number,
                     "QSFP28-100G-AOC",
    ...
    *type_copper = false;
    ...
    LOG_DEBUG("forcing port %d qsfp type to optical %s p/n %s\n",
              ...
              vendor.part_number);
    return true;
  } else if (bf_qsfp_info_arr[port].special_case_port.is_set) {
    // Set by cli
    *qsfp_type = bf_qsfp_info_arr[port].special_case_port.qsfp_type;
    *type_copper = bf_qsfp_get_is_type_copper(*qsfp_type);
    vendor.name[15] = vendor.part_number[15] = 0;
    return true;
  }
...
static bool bf_qsfp_is_eth_ext_compliance_copper(uint8_t ext_comp) {
...
int bf_qsfp_type_get(int port, bf_pltfm_qsfp_type_t *qsfp_type) {
   ...
   */
  if (bf_qsfp_special_case_handled(port, qsfp_type, &type_copper)) {
    return 0;
  }
  ...
  if (eth_comp & 0x77) {  // all except 40GBASE-CR4 and ext compliance bits
    ...
    if (bf_qsfp_get_eth_ext_compliance(port, &ext_comp) == 0) {
      type_copper = bf_qsfp_is_eth_ext_compliance_copper(ext_comp);
    } else {
      type_copper = true;
    }
  }
  ...
  uint8_t cable_len;
  if (bf_qsfp_field_read_onebank(port, LENGTH_CBLASSY, 0, 0, 1, &cable_len) <
      0) {
    return -1;
  }
  switch (cable_len) {
...
void bf_qsfp_debug_clear_all_presence_bits(void) {
  ...
  for (port = 0; port < BF_PLAT_MAX_QSFP + 1; port++) {
    if (bf_qsfp_info_arr[port].present) {
...
bool bf_qsfp_is_luxtera_optic(int port) { return bf_pm_qsfp_is_luxtera(port); }
...
bool bf_qsfp_is_cable_assy(int port) {
  ...
  if (bf_qsfp_field_read_onebank(port, CONN_TYPE, 0, 0, 1, &ct) < 0) {
    return false;
  }
...
static const char *bf_cmis_get_cu_string(int port) {
...
static const char *bf_cmis_get_active_cbl_string(int port) {
  uint8_t media_tech;
  ...
  if (bf_qsfp_field_read_onebank(port, MEDIA_INT_TECH, 0, 0, 1, &media_tech) <
      0) {
    return "Unknown";
  }
...
static const char *bf_cmis_get_custom_media_type_string(int port) {
  uint8_t media_id, media_type;
  qsfp_vendor_info_t vendor;
  bf_qsfp_get_vendor_info(port, &vendor);

  if (cmis_get_media_type(port, &media_type) != 0) {
    return NULL;
  }

  if (bf_qsfp_field_read_onebank(port, APSEL1_MEDIA_ID, 0, 0, 1, &media_id) <
      0) {
    return NULL;
  }
...
const char *bf_cmis_get_media_type_string(int port) {
  uint8_t media_id, media_type;
  ...
  media_str = bf_cmis_get_custom_media_type_string(port);
  if (media_str) return media_str;
...
int bf_cmis_select_Application(int port,
  ...
  return bf_qsfp_field_write_onebank(
      port, APPLICATION_SELECT_LN1, (0x1 << chnum), (chnum % 8), 1, &data);
}
...
int bf_cmis_get_activeSet(int port,
  ...
  rc = bf_qsfp_field_read_onebank(
      port, LN1_ACTIVE_SET, (0x1 << chnum), (chnum % 8), 1, &data);
  if (rc) {
    return -1;
  }
...
int bf_cmis_get_datapath_state(int port,
  ...
  rc = bf_qsfp_field_read_onebank(port,
                                  ...
                                  &readval);
  if (rc == 0) {
    *datapath_state = (readval >> ((ch % 2) * 4)) & 0xF;
  }
  return rc;
}

int bf_cmis_get_datapath_config_status(
    int port, int ch, DataPath_Config_Status *datapath_config_status) {
  int rc;
  uint8_t readval;
  ...
  rc = bf_qsfp_field_read_onebank(port,
                                  ...
                                  &readval);
  if (rc == 0) {
    *datapath_config_status = (readval >> ((ch % 2) * 4)) & 0xF;
  }
  return rc;
}
...
int bf_cmis_get_module_state(int port, Module_State *module_state) {
  ...
  rc = bf_qsfp_field_read_onebank(port, MODULE_STATE, 0, 0, 1, &readval);
  if (rc == 0) {
...
int bf_qsfp_vec_init(bf_qsfp_vec_t *vec) {
  ...
  if (vec->pltfm_write) bf_qsfp_vec.pltfm_write = vec->pltfm_write;
  if (vec->pltfm_read) bf_qsfp_vec.pltfm_read = vec->pltfm_read;
...
void bf_qsfp_get_sff_eth_extended_code_description(int port,
  ...
  size_t asize = sizeof(sff_ext_code_map) / sizeof(sff_ext_code_map[0]);

  Ethernet_compliance eth_comp = COMPLIANCE_NONE;
  Ethernet_extended_compliance ext_comp = EXT_COMPLIANCE_NONE;

  bf_qsfp_get_eth_compliance(port, &eth_comp);
  bf_qsfp_get_eth_ext_compliance(port, &ext_comp);

  if (eth_comp & 0x77) {  // all except 40GBASE-CR4 and ext compliance bits
    sprintf(ext_code_desc, "Other Eth compliance 0x%0x", eth_comp);
    return;
  }

  if (!(eth_comp & COMPLIANCE_RSVD)) {  // extended
    sprintf(ext_code_desc, "Non extended Eth compliance 0x%0x", ext_comp);
    return;
  }

  while (asize) {
    if (cPtr->ext_code == ext_comp) {
      strncpy(ext_code_desc, &cPtr->ext_code_desc[0], 250);
      code_found = true;
      break;
    }
    cPtr++;
  ...
  if (!code_found) {
    sprintf(ext_code_desc, "Reserved (0x%0X)", ext_comp);
  }
}
