...
#endif
static bf_pltfm_bd_cfg_qsfp_type_fn qsfp_type_get = NULL;
...
*
******************************************************************************/
static bd_map_ent_t *pltfm_bd_map_find(uint32_t connector, uint32_t channel) {
...
// Public functions
uint32_t bf_bd_first_conn_ch_get(uint32_t connector) {
  pltfm_bd_map_t *bd_map;
  int bd_map_rows;
  uint32_t row;
...
bf_pltfm_status_t bf_bd_cfg_port_mac_get(bf_pltfm_port_info_t *port_info,
                                               uint32_t *mac_id,
  ...
  bd_map_ent_t *bd_map =
      pltfm_bd_map_find(port_info->conn_id, port_info->chnl_id);
...
bf_pltfm_status_t bf_bd_cfg_port_info_get(
    uint32_t mac_id, uint32_t mac_chnl_id, bf_pltfm_port_info_t *port_info) {
  pltfm_bd_map_t *bd_map;
  int bd_map_rows;
  uint32_t row;
  ...
  for (row = 0; row < bd_map->rows; row++) {
    if ((bd_map->bd_map[row].mac_block == mac_id) &&
      ...
      port_info->conn_id = bd_map->bd_map[row].connector;
...
bf_pltfm_status_t bf_bd_cfg_port_tx_phy_lane_get(
    bf_pltfm_port_info_t *port_info, uint32_t *tx_phy_lane_id) {
  if (NULL == port_info) {
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      tx_phy_lane_id[i] = bd_map->multi_lane_map[i].tx_lane;
    }
  } else {
...
bf_pltfm_status_t bf_bd_cfg_port_rx_phy_lane_get(
    bf_pltfm_port_info_t *port_info, uint32_t *rx_phy_lane_id) {
  if (NULL == port_info) {
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      rx_phy_lane_id[i] = bd_map->multi_lane_map[i].rx_lane;
    }
  } else {
...
bf_pltfm_status_t bf_bd_cfg_port_tx_pn_swap_get(
    bf_pltfm_port_info_t *port_info, bool *tx_phy_pn_swap) {
  if (NULL == port_info) {
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      tx_phy_pn_swap[i] = (bd_map->multi_lane_map[i].tx_pn_swap ? true : false);
    }
  } else {
    *tx_phy_pn_swap = (bd_map->tx_pn_swap ? true : false);
  }
...
bf_pltfm_status_t bf_bd_cfg_port_rx_pn_swap_get(
    bf_pltfm_port_info_t *port_info, bool *rx_phy_pn_swap) {
  if (NULL == port_info) {
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      rx_phy_pn_swap[i] = (bd_map->multi_lane_map[i].rx_pn_swap ? true : false);
    }
  } else {
    *rx_phy_pn_swap = (bd_map->rx_pn_swap ? true : false);
  }

  return BF_PLTFM_SUCCESS;
}

bf_pltfm_status_t bf_bd_cfg_port_tx_lane_attn_get(
  ...
  if (qsfp_type >= BF_PLTFM_QSFP_UNKNOWN) {
    qsfp_type = BF_PLTFM_QSFP_CU_LOOP; /* Default to loopback */
  }
...
bf_pltfm_status_t bf_bd_cfg_port_tx_lane_pre_emp_get(
    bf_pltfm_port_info_t *port_info,
    bf_pltfm_qsfp_type_t qsfp_type,
    uint32_t *pre_emp) {
  if (NULL == port_info) {
...
bf_pltfm_status_t bf_bd_cfg_port_tx_lane_post_emp_get(
    bf_pltfm_port_info_t *port_info,
    bf_pltfm_qsfp_type_t qsfp_type,
    uint32_t *post_emp) {
  if (NULL == port_info) {
...
bf_pltfm_status_t bf_bd_cfg_port_mac_lane_info_get(
    bf_pltfm_port_info_t *port_info,
    bf_pltfm_qsfp_type_t qsfp_type,
...
bf_pltfm_status_t bf_pltfm_bd_cfg_port_mac_lane_info_set(
    bf_pltfm_port_info_t *port_info,
    bf_pltfm_qsfp_type_t qsfp_type,
    bf_pltfm_mac_lane_info_t *lane_info) {
  if (NULL == port_info || NULL == lane_info) {
...
bf_pltfm_status_t bf_bd_cfg_port_mac_lane_info_update(
    bf_pltfm_port_info_t *port_info, bf_pltfm_mac_lane_info_t *lane_info) {
  int qsfp_id;
  ...
  for (qsfp_id = 0; qsfp_id < MAX_QSFP_TYPES; qsfp_id++) {
    bd_map->tx_lane = lane_info->tx_phy_lane_id;
    bd_map->rx_lane = lane_info->rx_phy_lane_id;
    bd_map->tx_pn_swap = lane_info->tx_phy_pn_swap ? 1 : 0;
    bd_map->rx_pn_swap = lane_info->rx_phy_pn_swap ? 1 : 0;
...
bf_pltfm_status_t bf_bd_cfg_port_has_rptr(bf_pltfm_port_info_t *port_info,
                                                bool *has_rptr) {
  if (NULL == port_info) {
    return BF_PLTFM_INVALID_ARG;
  }
...
bf_pltfm_status_t bf_bd_cfg_rptr_addr_get(bf_pltfm_port_info_t *port_info,
...
bf_pltfm_status_t bf_pltfm_bd_cfg_rptr_info_get(
  ...
  if (mode == BF_PLTFM_RPTR_EGRESS) {
    ...
    rptr_info->eq_bst1 = bd_map->eg_rptr_eq_bst1[qsfp_type];
    rptr_info->eq_bst2 = bd_map->eg_rptr_eq_bst2[qsfp_type];
    rptr_info->eq_bw = bd_map->eg_rptr_eq_bw[qsfp_type];
    rptr_info->eq_bypass_bst1 = bd_map->eg_rptr_eq_bst_bypass[qsfp_type];
    rptr_info->vod = bd_map->eg_rptr_vod[qsfp_type];
    rptr_info->regF = bd_map->eg_rptr_regF[qsfp_type];
    rptr_info->high_gain = bd_map->eg_rptr_high_gain[qsfp_type];
  } else if (mode == BF_PLTFM_RPTR_INGRESS) {
    rptr_info->rptr_num = bd_map->ig_rptr_nbr;
    rptr_info->chnl_num = bd_map->ig_rptr_port;
    rptr_info->eq_bst1 = bd_map->ig_rptr_eq_bst1[qsfp_type];
    rptr_info->eq_bst2 = bd_map->ig_rptr_eq_bst2[qsfp_type];
    rptr_info->eq_bw = bd_map->ig_rptr_eq_bw[qsfp_type];
    rptr_info->eq_bypass_bst1 = bd_map->ig_rptr_eq_bst_bypass[qsfp_type];
    rptr_info->vod = bd_map->ig_rptr_vod[qsfp_type];
    rptr_info->regF = bd_map->ig_rptr_regF[qsfp_type];
    rptr_info->high_gain = bd_map->ig_rptr_high_gain[qsfp_type];
  } else {
    return BF_PLTFM_INVALID_ARG;
  }
...
bf_pltfm_status_t bf_bd_cfg_port_has_rtmr(bf_pltfm_port_info_t *port_info,
                                                bool *has_rtmr) {
...
bf_pltfm_status_t bf_bd_cfg_rtmr_info_get(
    bf_pltfm_port_info_t *port_info, bf_pltfm_rtmr_info_t *rtmr_info) {
  ...
  rtmr_info->host_side_lane = bd_map->rtmr_host_side_lane;
  rtmr_info->host_side_tx_pn_swap = bd_map->rtmr_host_side_tx_pn_swap;
  rtmr_info->host_side_rx_pn_swap = bd_map->rtmr_host_side_rx_pn_swap;
  rtmr_info->line_side_lane = bd_map->rtmr_line_side_lane;
  rtmr_info->line_side_tx_pn_swap = bd_map->rtmr_line_side_tx_pn_swap;
  rtmr_info->line_side_rx_pn_swap = bd_map->rtmr_line_side_rx_pn_swap;
  rtmr_info->tx_main_opt = bd_map->rtmr_tx_main_opt;
  rtmr_info->tx_precursor_opt = bd_map->rtmr_tx_precursor_opt;
  rtmr_info->tx_post_1_opt = bd_map->rtmr_tx_post_1_opt;
...
int bf_bd_cfg_bd_num_port_get() { return num_board_ports; }

void pltfm_port_info_to_str(bf_pltfm_port_info_t *port_info,
                            char *str_hdl,
                            uint32_t str_len) {
...
bf_pltfm_status_t pltfm_port_str_to_info(const char *str,
                                         bf_pltfm_port_info_t *port_info) {
  int len, i;
  char *tmp_str, *ptr;
  bf_pltfm_status_t sts = BF_PLTFM_SUCCESS;

  if (!str || !port_info) return BF_PLTFM_INVALID_ARG;

  len = strlen(str);
  tmp_str = (char *)bf_sys_malloc(sizeof(str) * (len + 1));
  if (tmp_str == NULL) {
    return BF_PLTFM_INVALID_ARG;
  }
...
bf_pltfm_status_t bf_bd_cfg_qsfp_type_fn_reg(
  ...
  if (!fn) return BF_PLTFM_INVALID_ARG;
  qsfp_type_get = fn;
  return BF_PLTFM_SUCCESS;
}

bf_pltfm_status_t pltfm_port_qsfp_type_get(bf_pltfm_port_info_t *port_info,
                                           bf_pltfm_qsfp_type_t *qsfp_type) {
  if (!qsfp_type_get) {
    assert(0);
  }
...
bf_pltfm_status_t bf_bd_cfg_serdes_info_set(
    bf_pltfm_port_info_t *port_info, bf_pltfm_mac_lane_info_t *serdes_info) {
  ...
  sts = pltfm_port_qsfp_type_get(port_info, &qsfp_type);
 ...
 */
int bf_bd_is_this_port_internal(uint32_t conn_id, uint32_t chnl_id) {
...
#if 0
  // set via ucli
  if (bf_qsfp_internal_port_get(conn_id)) {
     return 1;
  }
#endif
 ...
 */
int bf_bd_is_this_channel_valid(uint32_t conn_id, uint32_t chnl_id) {
...
bf_pltfm_status_t bf_bd_port_serdes_tx_params_get(
    bf_pltfm_port_info_t *port_info,
  ...
  if (qsfpdd_type >= BF_PLTFM_QSFPDD_UNKNOWN) {
    LOG_ERROR("QSFPDD: Serdes TX params requested for unknown module type "
        ...
        port_info->conn_id, port_info->chnl_id, encoding, qsfpdd_type);
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (NULL == bd_map) {
    ...
    tx_eq_dst->tx_main = bd_map->tx_eq_for_qsfpdd[encoding]->tx_main[qsfpdd_type];
    tx_eq_dst->tx_pre1 = bd_map->tx_eq_for_qsfpdd[encoding]->tx_pre1[qsfpdd_type];
    tx_eq_dst->tx_pre2 = bd_map->tx_eq_for_qsfpdd[encoding]->tx_pre2[qsfpdd_type];
    tx_eq_dst->tx_post1 = bd_map->tx_eq_for_qsfpdd[encoding]->tx_post1[qsfpdd_type];
    tx_eq_dst->tx_post2 = bd_map->tx_eq_for_qsfpdd[encoding]->tx_post2[qsfpdd_type];
   
  return BF_PLTFM_SUCCESS;
}

bf_pltfm_status_t bf_bd_port_serdes_polarity_get(
    bf_pltfm_port_info_t *port_info, int *rx_inv, int *tx_inv) {

  if (NULL == port_info || NULL == rx_inv || NULL == tx_inv) {
    return BF_PLTFM_INVALID_ARG;
  }
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      tx_inv[i] = bd_map->multi_lane_map[i].tx_pn_swap;
      rx_inv[i] = bd_map->multi_lane_map[i].rx_pn_swap;
    }
  } else {
 ...
 * returns dev-id
 */
uint32_t bf_bd_get_dev_id(uint32_t conn_id, uint32_t chnl_id) {
...
bf_pltfm_status_t bf_bd_cfg_port_nlanes_per_ch_get(
    bf_pltfm_port_info_t *port_info, uint32_t *num_lanes) {
...
bf_pltfm_status_t bf_bd_cfg_port_is_multilane_channel_get(
    bf_pltfm_port_info_t *port_info, bool *is_multi_lanes_per_ch) {
  if ((!is_multi_lanes_per_ch) || (NULL == port_info)) {
    return BF_PLTFM_INVALID_ARG;
  }
...
bf_pltfm_status_t bf_bd_cfg_qsfp_ch_get(
    bf_pltfm_port_info_t *port_info, uint32_t *qsfp_ch) {
  ...
  if (bd_map->is_multi_lanes_per_ch) {
    for (uint32_t i = 0; i < bd_map->nlanes_per_ch; i++) {
      qsfp_ch[i] = bd_map->multi_lane_map[i].qsfp_channel;
    }
  } else {
