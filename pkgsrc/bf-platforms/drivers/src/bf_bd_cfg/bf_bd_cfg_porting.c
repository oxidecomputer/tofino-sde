...
bf_status_t bf_bd_cfg_front_panel_port_get_first(
  ...
  sts = bf_bd_cfg_port_mac_get(&port_info, &log_mac_id, &log_mac_lane);
  ...
  dev_id = (bf_dev_id_t)bf_bd_get_dev_id(port_info.conn_id, port_info.chnl_id);
  ...
  pltfm_port_info_to_str(
      &port_info, first_port_info->port_str, sizeof(first_port_info->port_str));
...
bf_status_t bf_bd_cfg_front_panel_port_get_next(
  ...
  bf_pltfm_status_t sts = BF_PLTFM_SUCCESS;
  bf_pltfm_port_info_t curr_pltfm_port_info, next_pltfm_port_info;
  int num_ports;
  uint32_t log_mac_id, log_mac_lane;
  bf_dev_id_t dev_id;
  uint32_t nlanes_per_ch = 0;
  ...
  if (!curr_port_info) {
    ...
    return bf_bd_cfg_front_panel_port_get_first(next_port_info);
  }
  ...
  curr_pltfm_port_info.conn_id = curr_port_info->port_hdl.conn_id;
  curr_pltfm_port_info.chnl_id = curr_port_info->port_hdl.chnl_id;

  // Derive the next pltfm port info
  next_pltfm_port_info.conn_id = curr_pltfm_port_info.conn_id;
  bf_bd_cfg_port_nlanes_per_ch_get(&curr_pltfm_port_info, &nlanes_per_ch);
  next_pltfm_port_info.chnl_id = curr_pltfm_port_info.chnl_id + nlanes_per_ch;
   ...
   */
  if (!bf_bd_is_this_channel_valid(next_pltfm_port_info.conn_id, next_pltfm_port_info.chnl_id)) {
    if ((next_pltfm_port_info.conn_id + 1) > ((uint32_t)num_ports)) {
      // last entry
      return BF_OBJECT_NOT_FOUND;
    }
    /* reset the channel and check for next connector */
    next_pltfm_port_info.conn_id = next_pltfm_port_info.conn_id + 1;
    next_pltfm_port_info.chnl_id = 0;
  }

  sts = bf_bd_cfg_port_mac_get(
      &next_pltfm_port_info, &log_mac_id, &log_mac_lane);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
        "Unable to get the mac id and chnl for front port %d/%d : %s (%d)",
  ...
  dev_id = (bf_dev_id_t)bf_bd_get_dev_id(next_pltfm_port_info.conn_id, next_pltfm_port_info.chnl_id);
  next_port_info->dev_id = dev_id;
  next_port_info->port_hdl.conn_id = next_pltfm_port_info.conn_id;
  next_port_info->port_hdl.chnl_id = next_pltfm_port_info.chnl_id;
...
bf_status_t bf_bd_cfg_port_str_to_handle(
  ...
  sts = pltfm_port_str_to_info(port_str, &port_info);
  if (sts != BF_PLTFM_SUCCESS) {
...
bf_status_t bf_bd_cfg_mac_to_serdes_map_get(
  ...
  sts = bf_bd_cfg_port_tx_phy_lane_get(&port_info, &tx_lane);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
  ...
  sts = bf_bd_cfg_port_rx_phy_lane_get(&port_info, &rx_lane);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
...
bf_status_t bf_bd_cfg_mac_to_multi_serdes_map_get(
    bf_pal_front_port_handle_t *port_hdl,
  ...
  sts = bf_bd_cfg_port_nlanes_per_ch_get(&port_info, &mac_blk_map->num_serdes_per_lane);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
  ...
  sts = bf_bd_cfg_port_tx_phy_lane_get(&port_info, &mac_blk_map->tx_lane[0]);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
        "Unable to get the serdes Tx lane connected to front port %d/%d : %s "
        "(%d)",
  ...
  sts = bf_bd_cfg_port_rx_phy_lane_get(&port_info, &mac_blk_map->rx_lane[0]);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
        "Unable to get the serdes Rx lane connected to front port %d/%d : %s "
        "(%d)",
  ...
  sts = bf_bd_port_serdes_polarity_get(&port_info, &mac_blk_map->rx_inv[0], &mac_blk_map->tx_inv[0]);
  if (sts != BF_PLTFM_SUCCESS) {
    LOG_ERROR(
